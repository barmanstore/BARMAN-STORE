let pgLib = null;

const parseBooleanEnv = (value, fallback = false) => {
  const raw = String(value ?? '')
    .trim()
    .toLowerCase();
  if (!raw) return fallback;
  return ['1', 'true', 'yes', 'on'].includes(raw);
};

const isVercelRuntime = () =>
  parseBooleanEnv(process.env.VERCEL, false) ||
  Boolean(String(process.env.NOW_REGION || '').trim());

const parseNumberEnv = (value, fallback) => {
  const parsed = Number(value);
  return Number.isFinite(parsed) && parsed > 0 ? parsed : fallback;
};

const resolvePrimaryConnectionString = () =>
  String(
    process.env.SUPABASE_DB_URL ||
      process.env.DATABASE_URL ||
      process.env.POSTGRES_URL ||
      process.env.POSTGRES_PRISMA_URL ||
      process.env.PG_CONNECTION_STRING ||
      ''
  ).trim();

const resolveFallbackConnectionString = () =>
  String(
    process.env.POSTGRES_FALLBACK_URL ||
      process.env.LOCAL_DB_URL ||
      process.env.LOCAL_DATABASE_URL ||
      process.env.POSTGRES_FALLBACK_CONNECTION_STRING ||
      ''
  ).trim();

const buildBasePoolConfig = () => {
  const vercelRuntime = isVercelRuntime();
  const defaultPoolLimit = vercelRuntime ? 2 : 10;
  const defaultIdleTimeoutMs = vercelRuntime ? 5000 : 30000;
  const defaultConnectionTimeoutMs = vercelRuntime ? 10000 : 10000;
  const poolLimitRaw = Number(process.env.PG_POOL_LIMIT || defaultPoolLimit);

  return {
    vercelRuntime,
    base: {
      // Serverless runtimes should keep pool fan-out minimal or they can exhaust
      // the shared Postgres connection budget during concurrent cold starts.
      max: Number.isFinite(poolLimitRaw) && poolLimitRaw > 0 ? poolLimitRaw : defaultPoolLimit,
      idleTimeoutMillis: Number(process.env.PG_IDLE_TIMEOUT_MS || defaultIdleTimeoutMs),
      connectionTimeoutMillis: Number(
        process.env.PG_CONNECTION_TIMEOUT_MS || defaultConnectionTimeoutMs
      ),
    },
  };
};

const buildConfigFromConnectionString = ({ connectionString, sslEnabledFromEnv } = {}) => {
  const { base } = buildBasePoolConfig();
  const sslEnabled =
    sslEnabledFromEnv === undefined
      ? Boolean(connectionString && /supabase\.(co|com)/i.test(connectionString))
      : parseBooleanEnv(sslEnabledFromEnv, false);
  const rejectUnauthorized = parseBooleanEnv(
    process.env.PG_SSL_REJECT_UNAUTHORIZED ?? process.env.SUPABASE_DB_SSL_REJECT_UNAUTHORIZED,
    false
  );

  return {
    ...base,
    connectionString,
    ...(sslEnabled ? { ssl: { rejectUnauthorized } } : {}),
  };
};

const buildNamedPostgresConfigFromEnv = ({ useFallback = false } = {}) => {
  const { vercelRuntime, base } = buildBasePoolConfig();
  const connectionString = useFallback
    ? resolveFallbackConnectionString()
    : resolvePrimaryConnectionString();

  const sslEnabledFromEnv = process.env.PG_SSL ?? process.env.SUPABASE_DB_SSL;

  if (connectionString) {
    return buildConfigFromConnectionString({ connectionString, sslEnabledFromEnv });
  }

  const host = String(
    useFallback
      ? process.env.POSTGRES_FALLBACK_HOST ||
          process.env.LOCAL_DB_HOST ||
          process.env.LOCAL_DATABASE_HOST ||
          ''
      : process.env.PGHOST || process.env.PG_HOST || process.env.POSTGRES_HOST || ''
  ).trim();
  const user = String(
    useFallback
      ? process.env.POSTGRES_FALLBACK_USER ||
          process.env.LOCAL_DB_USER ||
          process.env.LOCAL_DATABASE_USER ||
          ''
      : process.env.PGUSER || process.env.PG_USER || process.env.POSTGRES_USER || ''
  ).trim();
  const password = String(
    useFallback
      ? process.env.POSTGRES_FALLBACK_PASSWORD ||
          process.env.LOCAL_DB_PASSWORD ||
          process.env.LOCAL_DATABASE_PASSWORD ||
          ''
      : process.env.PGPASSWORD || process.env.PG_PASSWORD || process.env.POSTGRES_PASSWORD || ''
  );
  const database = String(
    useFallback
      ? process.env.POSTGRES_FALLBACK_DATABASE ||
          process.env.LOCAL_DB_NAME ||
          process.env.LOCAL_DATABASE_NAME ||
          ''
      : process.env.PGDATABASE || process.env.PG_DATABASE || process.env.POSTGRES_DATABASE || ''
  ).trim();
  const port = parseNumberEnv(
    useFallback
      ? process.env.POSTGRES_FALLBACK_PORT ||
          process.env.LOCAL_DB_PORT ||
          process.env.LOCAL_DATABASE_PORT ||
          5432
      : process.env.PGPORT || process.env.PG_PORT || process.env.POSTGRES_PORT || 5432,
    5432
  );

  if (useFallback && !host) {
    return null;
  }

  if (vercelRuntime && !host) {
    throw new Error(
      '[DB] Missing SUPABASE_DB_URL/DATABASE_URL/POSTGRES_URL (or PGHOST-based settings) in Vercel environment.'
    );
  }

  const sslEnabled = useFallback
    ? parseBooleanEnv(process.env.POSTGRES_FALLBACK_SSL, false)
    : parseBooleanEnv(sslEnabledFromEnv, false);
  const rejectUnauthorized = useFallback
    ? parseBooleanEnv(process.env.POSTGRES_FALLBACK_SSL_REJECT_UNAUTHORIZED, false)
    : parseBooleanEnv(
        process.env.PG_SSL_REJECT_UNAUTHORIZED ?? process.env.SUPABASE_DB_SSL_REJECT_UNAUTHORIZED,
        false
      );

  return {
    ...base,
    host: host || '127.0.0.1',
    port,
    user: user || 'postgres',
    password,
    database: database || 'postgres',
    ...(sslEnabled ? { ssl: { rejectUnauthorized } } : {}),
  };
};

const buildPostgresConfigFromEnv = () => buildNamedPostgresConfigFromEnv({ useFallback: false });

const formatPostgresConnectionLabel = (cfg) => {
  if (!cfg) return 'postgres@unconfigured';
  if (cfg.connectionString) {
    try {
      const parsed = new URL(cfg.connectionString);
      const user = decodeURIComponent(parsed.username || 'postgres');
      const host = parsed.hostname || 'localhost';
      const port = parsed.port || '5432';
      const db = (parsed.pathname || '/postgres').replace(/^\//, '') || 'postgres';
      return `${user}@${host}:${port}/${db}`;
    } catch (_) {
      return 'postgres@connection-string';
    }
  }
  return `${cfg.user}@${cfg.host}:${cfg.port}/${cfg.database}`;
};

const getPostgresConnectionLabel = (cfg = null) =>
  formatPostgresConnectionLabel(cfg || buildPostgresConfigFromEnv());

const getPostgresConnectionCandidatesFromEnv = () => {
  const candidates = [];
  const primaryConfig = buildNamedPostgresConfigFromEnv({ useFallback: false });
  if (primaryConfig) {
    candidates.push({
      name: 'primary',
      config: primaryConfig,
      label: formatPostgresConnectionLabel(primaryConfig),
    });
  }

  const fallbackConfig = buildNamedPostgresConfigFromEnv({ useFallback: true });
  if (fallbackConfig) {
    candidates.push({
      name: 'fallback',
      config: fallbackConfig,
      label: formatPostgresConnectionLabel(fallbackConfig),
    });
  }

  return candidates;
};

const loadPostgresLibrary = () => {
  if (pgLib) return pgLib;
  try {
    pgLib = require('pg');
  } catch (_) {
    throw new Error(
      'pg package is not installed. Run "npm install pg" before enabling postgres/supabase mode.'
    );
  }
  return pgLib;
};

const createPostgresPool = (cfg = null) =>
  new (loadPostgresLibrary().Pool)(cfg || buildPostgresConfigFromEnv());

const pingPostgresPool = async (pool) => {
  const result = await pool.query('SELECT 1 AS ok');
  if (Number(result?.rows?.[0]?.ok || 0) !== 1) {
    throw new Error('Unexpected postgres ping response');
  }
  return true;
};

module.exports = {
  buildPostgresConfigFromEnv,
  createPostgresPool,
  getPostgresConnectionCandidatesFromEnv,
  getPostgresConnectionLabel,
  pingPostgresPool,
};
