...
const createPurchaseOperationsRollups = (deps) => {
  ...
  const buildPurchaseActionRollupsAsync = async ({
    ...
    const poCreatedRows = await rollupQueries.fetchPoCreatedRows({
      normalizedStart,
      normalizedEnd,
      distributorId,
    });
    poCreatedRows.forEach((row) =>
      rollupState.addCount(byDayMap, row.action_date, 'po_created', 1)
    );

    const statusRows = await rollupQueries.fetchStatusRows({
      normalizedStart,
      normalizedEnd,
      distributorId,
    });
    ...
    const paymentRows = await rollupQueries.fetchPaymentRows({
      normalizedStart,
      normalizedEnd,
      distributorId,
    });
    ...
    const deliveryRows = await rollupQueries.fetchDeliveryRows({
      normalizedStart,
      normalizedEnd,
      distributorId,
    });
    deliveryRows.forEach((row) =>
      rollupState.addCount(byDayMap, row.action_date, 'delivery_received', 1)
    );

    const returnRows = await rollupQueries.fetchReturnRows({
      normalizedStart,
      normalizedEnd,
      distributorId,
    });
    ...
    const ledgerRows = await rollupQueries.fetchLedgerRows({
      normalizedStart,
      normalizedEnd,
      distributorId,
    });
    ledgerRows.forEach((row) =>
      rollupState.addCount(byDayMap, row.action_date, 'ledger_manual', 1)
    );

    const reminderRows = await rollupQueries.fetchReminderRows({
      normalizedStart,
      normalizedEnd,
      distributorId,
    });
    reminderRows.forEach((row) =>
      rollupState.addCount(byDayMap, row.action_date, 'reminder_sent', 1)
    );
