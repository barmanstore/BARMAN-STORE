...
const getScoreBand = (score) => {
  ...
  return (
    SORTED_SCORE_BANDS.find((band) => numericScore >= band.min) ||
    SORTED_SCORE_BANDS[SORTED_SCORE_BANDS.length - 1] ||
    null
  );
};
...
const getBandIndex = (key) => {
  const normalizedKey = String(key || '')
    .trim()
    .toLowerCase();
  return SORTED_SCORE_BANDS.findIndex(
    (band) =>
      String(band?.key || '')
        .trim()
        .toLowerCase() === normalizedKey
...
const getCycleBandForElapsedDays = (elapsedDays = 0, { graceDays = DEFAULT_GRACE_DAYS } = {}) => {
  const normalizedElapsedDays = Math.max(0, Math.floor(toFiniteNumber(elapsedDays, 0)));
  const normalizedGraceDays = Math.max(
    0,
    Math.floor(toFiniteNumber(graceDays, DEFAULT_GRACE_DAYS))
  );
...
const getCycleBandWindow = (key, { graceDays = DEFAULT_GRACE_DAYS } = {}) => {
  ...
  const lower =
    currentIndex <= 0
      ? 0
      : Math.max(0, Math.floor(Number(SORTED_SCORE_BANDS[currentIndex - 1]?.target_days || 0))) +
        normalizedGraceDays +
        1;
...
const getBandScoreForElapsedDays = (
  key,
  elapsedDays = 0,
...
const isNewCustomerSummary = (summary = {}) => {
  const customerTag = String(summary?.customer_tag || '')
    .trim()
    .toLowerCase();
  const paymentStatus = String(summary?.payment_status || summary?.payment_status_key || '')
    .trim()
    .toLowerCase();
  const paymentStatusLabel = String(summary?.payment_status_label || summary?.label || '')
    .trim()
    .toLowerCase();
  return (
    customerTag === 'insufficient_history' ||
    paymentStatus === 'new' ||
...
const resolveTargetBand = (summary = {}) => {
  ...
  return (
    getBandByKey(
      summary?.payment_status || summary?.payment_status_key || summary?.badge_key || ''
    ) || NEW_CUSTOMER_STATUS
  );
};

const resolveCreditTermsDays = ({ creditTermsDays, paymentSummary } = {}) => {
