const createPurchaseDuplicateUtils = (deps = {}) => {
  const { dbGetAsync, crypto, normalizeTransactionDate, PO_LIFECYCLE_CANCELLED } = deps;

  const buildPurchaseOrderFingerprint = (items = []) => {
    const normalized = (Array.isArray(items) ? items : [])
      .map((item) => {
        const productKey = String(item?.product_id || item?.product_name || 'custom')
          .trim()
          .toLowerCase();
        const quantity = Number(item?.quantity || 0).toFixed(3);
        const uom = String(item?.uom || 'pcs')
          .trim()
          .toLowerCase();
  ...
  const buildPurchaseDuplicateKey = ({ distributorId, plannedOrderDate, items = [] } = {}) => {
    ...
    const plannedDateKey =
      normalizeTransactionDate(plannedOrderDate || new Date().toISOString()) ||
  ...
  const findDuplicateDistributorBillAsync = async ({
    distributorId,
    billNumber,
    excludeOrderId = null,
  } = {}) => {
