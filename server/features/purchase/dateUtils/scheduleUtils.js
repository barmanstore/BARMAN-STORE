const createScheduleUtils = ({ normalizeWeekdayLabel }) => {
  const getDistributorVisitScheduleDay = (distributor = {}) =>
    normalizeWeekdayLabel(
      distributor.visit_day || distributor.delivery_day || distributor.order_day
    );
  ...
  const getSupplierScheduleConfig = (supplier = {}, distributor = {}) => {
    const rawType = String(supplier?.schedule_type || '')
      .trim()
      .toLowerCase();
    ...
    if (scheduleType === 'weekly') {
      ...
      const looksLikeLegacyOrderBackfill =
        Boolean(supplier?.is_primary) &&
        supplierScheduleDay &&
        distributorVisitDay &&
        distributorOrderDay &&
