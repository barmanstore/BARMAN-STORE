const createPurchaseOrderStatusUtils = (deps = {}) => {
  ...
  const PO_PAYMENT_ALLOWED_STATUSES = new Set([PO_LIFECYCLE_CONFIRMED, PO_LIFECYCLE_PART_PAID]);
  const PO_RECEIVE_ALLOWED_STATUSES = new Set([
    PO_LIFECYCLE_CONFIRMED,
    PO_LIFECYCLE_PART_PAID,
    PO_LIFECYCLE_FULLY_PAID,
  ]);
  ...
  const normalizePoLifecycleStatus = (status, fallback = PO_LIFECYCLE_PREPARED) => {
    ...
    if (raw === PO_LIFECYCLE_FULLY_PAID || raw === 'full_paid' || raw === 'paid')
      return PO_LIFECYCLE_FULLY_PAID;
  ...
  const normalizePoPaymentStatus = (value, fallback = PO_PAYMENT_UNPAID) => {
    ...
    if (raw === PO_PAYMENT_PART_PAID || raw === 'partpaid' || raw === 'partial')
      return PO_PAYMENT_PART_PAID;
  ...
  const derivePoLifecycleFromPaymentStatus = (baseStatus, paymentStatus) => {
    ...
    if (normalizedBase === PO_LIFECYCLE_CANCELLED || normalizedBase === PO_LIFECYCLE_CLOSED)
  ...
  const isPoEditableLifecycle = (status) =>
    PO_EDITABLE_STATUSES.has(normalizePoLifecycleStatus(status));
  const canPoAcceptPayment = (status) =>
    PO_PAYMENT_ALLOWED_STATUSES.has(normalizePoLifecycleStatus(status));
  const canPoReceiveInventory = (status) =>
  ...
  const derivePurchaseNextAction = (order = {}) => {
    ...
    if (lifecycleStatus === PO_LIFECYCLE_SENT || lifecycleStatus === PO_LIFECYCLE_REVISED)
      return 'Confirm with bill';
  ...
  const recordPurchaseOrderStatusHistoryAsync = async (
    purchaseOrderId,
    {
      fromStatus = null,
      toStatus,
