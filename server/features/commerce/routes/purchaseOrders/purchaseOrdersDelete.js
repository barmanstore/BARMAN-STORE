  ...
  app.delete('/api/purchase-orders/:id', requireAdmin, async (req, res) => {
    try {
      const existing = await dbGetAsync(
        `SELECT id, po_status, status FROM purchase_orders WHERE id = ?`,
        [req.params.id]
      );
      ...
      if (!isPoEditableLifecycle(getPurchaseOrderLifecycleStatus(existing))) {
        return res
          .status(400)
          .json({ error: 'Only prepared, sent, or revised purchase orders can be deleted' });
      }
      await dbRunAsync(`DELETE FROM purchase_order_payments WHERE purchase_order_id = ?`, [
        req.params.id,
      ]);
