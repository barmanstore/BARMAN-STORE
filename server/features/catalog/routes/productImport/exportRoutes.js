...
const PRODUCT_EXPORT_SORT_SPECS = {
  ...
  brand: { expr: "LOWER(TRIM(COALESCE(p.brand, '') || CASE WHEN COALESCE(p.sub_brand, '') = '' THEN '' ELSE ' -> ' || COALESCE(p.sub_brand, '') END))" },
  category: { expr: "LOWER(TRIM(COALESCE(p.category, '') || CASE WHEN COALESCE(p.subcategory, '') = '' THEN '' ELSE ' -> ' || COALESCE(p.subcategory, '') END))" },
...
const registerProductExportRoutes = (deps) => {
  ...
  app.get('/api/products/export', requireAdmin, async (req, res) => {
    try {
      ...
      const searchQuery = applyFilters ? normalizeQueryText(req.query?.q || req.query?.name || '').toLowerCase() : '';
      ...
      const rawStatus = applyFilters ? String(req.query?.status || '').trim().toLowerCase() : '';
      const lowStockOnly = applyFilters && String(req.query?.low_stock || '').trim().toLowerCase() === 'true';
      const sortField = normalizeSortField(req.query?.sort_field || '') || 'created_at';
      const sortDir = normalizeSortDir(req.query?.sort_dir || '') || (sortField === 'name' || sortField === 'brand' || sortField === 'category' || sortField === 'sku' ? 'asc' : 'desc');
