const registerContactVerificationAdminApproveRoutes = (deps) => {
  ...
  app.post(
    '/api/admin/contact-verification-requests/:id/approve-send',
    requireAdmin,
    async (req, res) => {
      try {
        const requestId = Number(req.params.id || 0);
        if (!requestId) return res.status(400).json({ error: 'Invalid request id' });
        const requestRow = await dbGetAsync(
          'SELECT * FROM contact_verification_requests WHERE id = ?',
          [requestId]
        );
        ...
        if (!requestType)
          return res.status(400).json({ error: 'Unsupported verification request type' });
        ...
        return res
          .status(500)
          .json({ error: error.message || 'Failed to approve verification request' });
      }
    }
  );
};
