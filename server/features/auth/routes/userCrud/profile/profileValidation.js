const validateProfileUpdateInputs = async ({
  ...
  const { name, email, phone, address, profile_image, role, credit_limit: creditLimitRaw } = input || {};
  if (!current) {
  ...
  if (isAdmin && !isSelf) {
    ...
    if (role !== undefined) {
      nextRole = String(role || current.role).trim().toLowerCase() === 'admin' ? 'admin' : 'customer';
