...
const resolveLinkedOrderItemForBilling = ({
  ...
  if (explicitLinkedOrderItemId > 0) {
    ...
    if (!matchedById) {
      throw createHttpError(
        400,
        `Linked order item ${explicitLinkedOrderItemId} is invalid for this order`
      );
    }
    if (usedLinkedOrderItemIds.has(explicitLinkedOrderItemId)) {
      throw createHttpError(
        400,
        `Linked order item ${explicitLinkedOrderItemId} is duplicated in this bill`
      );
    }
  ...
  if (matchedOrderItemId > 0) {
    if (usedLinkedOrderItemIds.has(matchedOrderItemId)) {
      throw createHttpError(
        400,
        `Linked order item ${matchedOrderItemId} is duplicated in this bill`
      );
    }
...
const buildBillItems = async (deps, context, createHttpError) => {
  const { dbAllAsync, toStockUnitQty, fromStockUnitQty, roundQty } = deps;
  const { items, billType, fulfillmentMode, customer, linkedOrderId, linkedOrderItems } = context;
  ...
  if (linkedOrderId) {
    ...
    pricedItems = sanitizedItems.map((item) => {
      ...
      return {
        ...
        product_name:
          String(linkedOrderItem?.product_name || item?.product_name || 'Item').trim() || 'Item',
        qty: requestedQty,
    ...
    const offerPreviewByClientId = new Map(
      (Array.isArray(offerPreview?.items) ? offerPreview.items : []).map((line) => [
        String(line?.client_item_id),
        line,
      ])
    );
    pricedItems = sanitizedItems.map((item) => {
      ...
      if (!line) {
        ...
        const manualDiscount = Math.min(
          lineSubtotal,
          Math.max(0, Number(item.manual_discount || 0))
        );
        return {
          ...item,
          line_subtotal: lineSubtotal,
