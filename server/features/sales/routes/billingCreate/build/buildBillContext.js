const buildBillContext = async (deps, req, createHttpError) => {
  ...
  if (linkedOrderId) {
    ...
    const existingForOrder = await dbGetAsync(
    ...
    linkedOrderItems = await dbAllAsync(
  ...
  const billTypeRaw = String(b.bill_type || 'sales')
    .trim()
    .toLowerCase();
  const billType = billTypeRaw === 'purchase' ? 'purchase' : 'sales';
  const fulfillmentModeRaw = String(b.fulfillment_mode || '')
    .trim()
    .toLowerCase();
  const fulfillmentMode = linkedOrderId
    ? fulfillmentModeRaw === 'full_now'
      ? 'full_now'
      : 'available_now'
    : 'full_now';
  ...
  let customer = customerId
    ? await dbGetAsync('SELECT id, name, email, phone, address, role FROM users WHERE id = ?', [
        customerId,
      ])
    : null;
