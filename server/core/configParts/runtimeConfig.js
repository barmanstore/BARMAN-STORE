const createRuntimeConfig = ({
  ...
  const DB_EXECUTION_MODE = normalizeExecutionMode(
    env.DB_EXECUTION_MODE || env.DB_CLIENT || 'postgres'
  );
  ...
  const POSTGRES_MIGRATIONS_DIR =
    env.POSTGRES_MIGRATIONS_DIR || path.join(baseDir, '..', 'supabase', 'migrations');
  ...
  const CANONICAL_HOST = String(env.CANONICAL_HOST || 'barmanstore.vercel.app')
    .trim()
    .toLowerCase();
