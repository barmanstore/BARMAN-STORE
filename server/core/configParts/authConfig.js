const createAuthConfig = ({ env, parseBooleanEnv, defaultOnlineStoreUrl } = {}) => {
  ...
  const AUTH_FLOW_MODE =
    String(env.AUTH_FLOW_MODE || 'manual')
      .trim()
      .toLowerCase() === 'provider'
      ? 'provider'
      : 'manual';
  const PASSWORD_RESET_MODE =
    String(env.PASSWORD_RESET_MODE || (AUTH_FLOW_MODE === 'provider' ? 'otp' : 'admin'))
      .trim()
      .toLowerCase() === 'otp'
      ? 'otp'
      : 'admin';
  const PHONE_VERIFICATION_REQUIRED = parseBooleanEnv(env.PHONE_VERIFICATION_REQUIRED, false);
  const OTP_PROVIDER = String(env.OTP_PROVIDER || 'twilio')
    .trim()
    .toLowerCase();
  ...
  const AUTH_LOGIN_OTP_EXPOSE_CODE =
    env.NODE_ENV === 'production' ? false : AUTH_LOGIN_OTP_EXPOSE_CODE_REQUESTED;
  const OTP_DELIVERY_MODE =
    String(env.OTP_DELIVERY_MODE || (AUTH_FLOW_MODE === 'provider' ? 'auto' : 'manual'))
      .trim()
      .toLowerCase() === 'auto'
      ? 'auto'
      : 'manual';
  const EMAIL_DELIVERY_MODE =
    String(env.EMAIL_DELIVERY_MODE || 'manual')
      .trim()
      .toLowerCase() === 'auto'
      ? 'auto'
      : 'manual';
  const EMAIL_VERIFICATION_MODE =
    String(env.EMAIL_VERIFICATION_MODE || 'stub')
      .trim()
      .toLowerCase() === 'provider'
      ? 'provider'
      : 'stub';
  const WHATSAPP_DELIVERY_MODE =
    String(env.WHATSAPP_DELIVERY_MODE || 'manual')
      .trim()
      .toLowerCase() === 'auto'
      ? 'auto'
      : 'manual';
  const WHATSAPP_PROVIDER = String(env.WHATSAPP_PROVIDER || 'meta')
    .trim()
    .toLowerCase();
  const SUPABASE_AUTH_ENABLED = parseBooleanEnv(env.SUPABASE_AUTH_ENABLED, false);
  const SUPABASE_AUTH_MODE =
    String(env.SUPABASE_AUTH_MODE || 'hybrid')
      .trim()
      .toLowerCase() === 'strict'
      ? 'strict'
      : 'hybrid';
  ...
  const EMAIL_VERIFY_BASE_URL = String(
    env.EMAIL_VERIFY_BASE_URL || 'http://localhost/login'
  ).trim();
  const PHONE_VERIFY_BASE_URL = String(
    env.PHONE_VERIFY_BASE_URL || `${defaultOnlineStoreUrl}/login`
  ).trim();
  ...
  const OTP_VERIFY_SESSION_TTL_SECONDS = Math.max(
    60,
    Number(env.OTP_VERIFY_SESSION_TTL_SECONDS || 900)
  );
  const CREDIT_ENTRY_DEDUP_WINDOW_MS = Math.max(
    0,
    Number(env.CREDIT_ENTRY_DEDUP_WINDOW_MS || 15000)
  );
