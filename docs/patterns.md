...
- Treat [src/features/inventory/StockLedgerHistory.jsx](../src/features/inventory/StockLedgerHistory.jsx) as an assembly example, not the source of truth. The reusable contract should be split across the shared search bar, dropdown filter, date picker, stat card, table shell, page header, and empty-state stories so future ledger, audit, and review screens can be rebuilt from the same primitives without copying page chrome.
...
- When verifying a page that uses the shared filter/table system, use the same checklist every time: `SearchFilter`, `FilterBar`, `FilterTray`, `FilterRow`, `FilterPills`, `StatCard`, and `TableShell` should be the shared primitives, with page CSS limited to layout and table mechanics.
