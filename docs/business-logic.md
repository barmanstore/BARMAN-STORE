...
- Supplier-linked PO defaults should come from the persistent `supplier_products` registry, not from ad hoc purchase-history-only inference. Supplier selection should load that registry onto the PO board, keep those rows non-removable, and treat manually added extra products as a separate removable row type.
