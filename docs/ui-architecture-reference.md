...
- `FilterBar` is a flat layout rail, not a second card. Use it to align search, clear, and filter controls, but keep the visible chrome inside the shared filter controls themselves.
- The shared filter button family is part of that same contract. Use the shared `SearchFilterSubmitButton` and `FilterToggleButton` helpers so the submit icon and filter toggle read the same on every page.
