...
- A confirmed PO saved with `delivered=true` is treated as delivery-complete for workflow and next-action rendering, so the UI should not surface another `Receive delivery` step for that order.
