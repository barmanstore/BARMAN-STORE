...
- `POSTGRES_FALLBACK_URL`
  - Optional fallback connection string used when `SUPABASE_DB_URL`/`DATABASE_URL` is unavailable during runtime startup
  - Intended for local development failover to a local Postgres database
- `POSTGRES_FALLBACK_HOST` / `POSTGRES_FALLBACK_PORT` / `POSTGRES_FALLBACK_USER` / `POSTGRES_FALLBACK_PASSWORD` / `POSTGRES_FALLBACK_DATABASE`
  - Optional component-style fallback settings when you do not want to provide `POSTGRES_FALLBACK_URL`
- `POSTGRES_FALLBACK_SSL`
  - Default: `false`
  - Enables TLS for the fallback Postgres connection when the fallback database requires it
- `POSTGRES_FALLBACK_SSL_REJECT_UNAUTHORIZED`
  - Default: `false`
  - TLS certificate verification switch for the fallback Postgres connection
- `SUPABASE_URL`
...
- `SUPABASE_ANON_KEY`
  ...
  - The backend also accepts `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` or `NEXT_PUBLIC_SUPABASE_ANON_KEY` as a local fallback when you run `npm run server` directly
