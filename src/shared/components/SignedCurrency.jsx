...
function SignedCurrency({ amount, className = '' }) {
  ...
  const combinedClassName = [getSignedCurrencyClassName(safeAmount), className]
    .filter(Boolean)
    .join(' ');

  return <span className={combinedClassName}>{formatCurrency(Math.abs(safeAmount))}</span>;
}
