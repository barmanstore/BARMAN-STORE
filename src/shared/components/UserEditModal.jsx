...
function UserEditModal({ user, onClose, onSave, isCreate = false, createPrefill = null }) {
  const [formData, setFormData] = useState(() => ({
    ...
    credit_limit:
      isCreate && createPrefill?.credit_limit !== undefined && createPrefill?.credit_limit !== null
  ...
  const isRoleEditAllowed =
    isCreate || (Boolean(user?.email_verified) && Boolean(user?.phone_verified));
    ...
    <>
      ...
      <form onSubmit={handleSubmit} className="user-edit-form">
        {isCreate ? (
            ...
            <div className="form-group">
              ...
              <CalculatedAmountInput
                ...
                onValueChange={(nextValue) =>
                  handleChange({ target: { name: 'credit_limit', value: nextValue } })
                }
              ...
              <span className="info-text">
                Blank means unrestricted credit. Provided email or phone stays pending until
          ...
          <>
            <div className="user-static-summary">
              ...
              <p>
                <strong>Email status:</strong> {formatVerificationLabel(user?.email_verified)}
              </p>
              <p>
                <strong>Phone status:</strong> {formatVerificationLabel(user?.phone_verified)}
              </p>
              <p>
                <strong>Current credit limit:</strong> {formatCreditLimitLabel(user?.credit_limit)}
              </p>
            </div>
            <div className="user-edit-hint">
              Identity fields stay read-only in this admin edit flow. Email, phone, and verification
              changes must go through the contact verification flows so uniqueness checks and
              pending requests stay intact.
            </div>
            ...
            <div className="form-group">
              ...
              {isRoleEditAllowed && !isAdminTarget && (
                <span className="info-text">
                  Role changes stay available only while both email and phone remain verified.
                </span>
              )}
            </div>
            ...
            <div className="form-group">
              ...
              {!isAdminTarget ? (
                <span className="info-text">
                  Leave blank for unrestricted credit. Stored value `0` means no enforced limit.
                </span>
              ) : null}
            </div>
          </>
        ...
        <div className="form-actions">
          ...
          <button type="submit" className="submit-btn" disabled={loading || isAdminTarget}>
