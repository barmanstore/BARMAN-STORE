...
function useProductsRenderers({
  ...
  const renderQuickAddTile = useCallback(
    (family) => {
      const selectedVariation = getSelectedVariation(family);
      const cardState = getFamilyCardState(family, selectedVariation, cartQtyById);
    ...
    (family, { prioritizeImage = false } = {}) => {
      ...
      const unitPriceLabel =
        unitPriceValue > 0
          ...
          : '';
      const lowStockLabel =
        cardState.selectedStock > 0 && cardState.selectedStock <= LOW_STOCK_THRESHOLD
          ...
          detailContent={
            metaItems.length > 0 ? (
              <div className="mobile-card-meta">
                {metaItems.map((item) => (
                  <span key={`${family.id}-${item}`} className="mobile-card-meta-item">
                    {item}
                  </span>
                ))}
              </div>
