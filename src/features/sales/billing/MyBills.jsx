...
function MyBills() {
  ...
  useEffect(() => {
    ...
    const loadBills = async () => {
      ...
      setError('');
      try {
        const rows = await billingApi.getByUser(user.id);
        if (cancelled) return;
        const nextBills = Array.isArray(rows) ? rows : [];
        setBills(nextBills);
        if (!nextBills.length) setSelectedBill(null);
      } catch (err) {
    ...
    return (
      <MobileAccountLayout>
        <div className="my-bills-page">
          <div className="my-bills-loading">
            <RefreshCw size={20} className="spin" /> Loading bills...
          </div>
        </div>
      </MobileAccountLayout>
    );
        ...
        {bills.length === 0 ? (
          ...
          <div className="my-bills-layout">
              ...
              {bills.map((bill) => (
                ...
                >
                  <div className="my-bill-top">
                    ...
                    <span
                      className={`status ${String(bill.payment_status || 'pending').toLowerCase()}`}
                    >
            ...
            <section className="my-bill-detail">
              {detailsLoading ? (
                <div className="my-bills-loading">
                  <RefreshCw size={18} className="spin" /> Loading bill...
                </div>
              ) : !selectedBill ? (
                <div className="my-bills-empty">
                  ...
                  <div className="my-bill-items">
                    {(selectedBill.items || []).map((item) => (
                      <div key={item.id} className="my-bill-item">
                        ...
                        <span>
                          {item.qty} x {formatCurrency(Number(item.mrp || 0))}
                        </span>
