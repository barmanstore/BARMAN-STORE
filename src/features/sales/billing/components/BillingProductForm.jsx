...
const BillingProductForm = ({
  ...
  }, [activeProductSuggestionIndex, showRecentProducts, trimmedSearch, visibleProductResults]);
  ...
  return (
    <section className="billing-pos-panel billing-entry-panel">
      ...
      <div className="billing-entry-grid">
        <label
          className="billing-entry-field billing-entry-field-search"
        ...
        <label className="billing-entry-field" htmlFor="billing-product-qty">
          ...
          <input
            ...
            placeholder="1"
          />
          <small className="billing-entry-field-note">
            Enter saves. Shift+Enter goes to price.
          </small>
        </label>
      ...
      {currentProduct ? (
        <div className={`billing-entry-selection${pendingProductSelectionReview ? ' review' : ''}`}>
          ...
          {pendingProductSelectionReview ? <small>Press Enter in Qty to add.</small> : null}
        </div>
      ) : null}

      {lowStockWarning ? (
        <div
          className={`billing-entry-warning${lowStockWarning?.tone === 'danger' ? ' critical' : ''}`}
        >
          <strong>{lowStockWarning?.tone === 'danger' ? 'Stock:' : 'Low stock:'}</strong>{' '}
      ...
      <div className="billing-entry-actions">
        {isEditing ? (
          <button type="button" className="billing-secondary-btn" onClick={onCancelEdit}>
            <RotateCcw size={16} />
            Cancel
          </button>
        ) : null}
