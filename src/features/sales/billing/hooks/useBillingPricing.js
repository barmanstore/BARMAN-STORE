...
import { getProductDefaultPrice, getStockWarningMeta } from '../utils/billingLineItemUtils';
...
const getLinkedOrderFulfilledQty = (item = {}) =>
  Math.max(
    0,
    Number(
      item?.linkedOrderFulfilledQty ??
        item?.linkedOrderAvailableNowQty ??
        item?.availableNowQty ??
        item?.fulfilledQty ??
        0
    ) || 0
  );

const getEffectiveBillingQty = ({
  item = {},
  linkedOrderId = 0,
  fulfillmentMode = 'full_now',
} = {}) => {
...
const useBillingPricing = ({
  ...
  const billingPreviewItems = useMemo(
    () =>
      billItems.map((item) => {
        ...
        const skipOffers =
          Boolean(item?.skipOffers) ||
          itemType === 'custom' ||
          (product ? currentPrice !== defaultPrice : false);
        return {
  ...
  const localSubtotalAmount = useMemo(
    () =>
      roundMoney(
  ...
  const localTotalDiscount = useMemo(
    () =>
      roundMoney(
        billItems.reduce((sum, item) => {
          const priceNum = Number(item.price) || 0;
  ...
  const paidAmountWarning =
    hasPaidAmountInput && !paidAmountEvaluation.valid
    ...
    totalBill <= 0
      ...
      : paidClamped <= 0
        ...
        : paidClamped >= totalBill
          ? 'full_payment'
          : 'partial_payment';
  const effectivePaymentMethod = paymentIntent === 'full_credit' ? 'credit' : selectedPaymentMethod;
  const isOrderLinked = Number(linkedOrderId || 0) > 0;
  const activeLineItemsCount = billItems.length;
  const paymentStatusLabel = creditAmount > 0 ? (paidClamped > 0 ? 'Partial' : 'Due') : 'Paid';
  ...
  const billDisplayItems = useMemo(
    () =>
      billItems.map((item) => {
        const product = getProductForLine(item);
        const effectiveQty = getEffectiveBillingQty({
        ...
        const fallbackManualDiscount = Math.max(
          0,
          Number(item?.prefilledManualDiscount || item?.disc || 0)
        );
        const fallbackTotalDiscount = Math.max(
          0,
          Number(
            item?.prefilledTotalDiscount || fallbackOfferDiscount + fallbackManualDiscount || 0
          )
        );
        const fallbackOfferLabel = String(item?.prefilledOfferLabel || '').trim();
        const resolvedAmount =
          effectiveQty <= 0 && !previewLine
            ? 0
            : Number(previewLine?.line_total ?? item.amount ?? 0);
        const stockWarning = isCustomItem ? null : getStockWarningMeta(product, pricingQty);
        const cost = Number(
          product?.buy_price ?? product?.cost_price ?? product?.purchase_price ?? 0
        );
        const profitValue =
          Number.isFinite(cost) && cost > 0 ? resolvedAmount - cost * pricingQty : null;
        const priceUnit =
          String(
            product?.base_unit || product?.uom || product?.unit || item.unit || 'pcs'
          ).trim() || 'pcs';
        const requestedQty = getLinkedOrderRequestedQty(item);
        ...
        return {
          ...
          isPartialLinkedBilling: Boolean(
            linkedOrderId && fulfillmentMode === 'available_now' && requestedQty > effectiveQty
          ),
          isCustomItem,
          isManualPrice:
          ...
          lineSubtotal: Number(
            previewLine?.line_subtotal ??
              (Number(item?.prefilledLineSubtotal || 0) > 0
                ? roundMoney(Number(item.prefilledLineSubtotal || 0) * qtyRatio)
                : 0)
          ),
          offerDiscount: Number(
            previewLine?.auto_offer_discount ?? roundMoney(fallbackOfferDiscount * qtyRatio)
          ),
          manualDiscount: Number(
            previewLine?.manual_discount ?? roundMoney(fallbackManualDiscount * qtyRatio)
          ),
          totalDiscount: Number(
            previewLine?.line_discount_total ?? roundMoney(fallbackTotalDiscount * qtyRatio)
          ),
  ...
  const createBillConfirmationSignature = useMemo(
    () =>
      JSON.stringify({
  ...
  const billingDraftDirty = Boolean(
    ...
    (String(customer?.name || '').trim() ||
      ...
      String(selectedPaymentMethod || 'cash')
        .trim()
        .toLowerCase() !== 'cash' ||
      String(fulfillmentMode || 'available_now')
        .trim()
        .toLowerCase() !== 'available_now' ||
