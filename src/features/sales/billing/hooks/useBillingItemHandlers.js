...
const canMergeBillItems = (existingItem = {}, nextItem = {}) => {
  if (
    getBillingItemType(existingItem) !== 'inventory' ||
  ...
  return (
    normalizeLookupKey(existingItem?.unit || 'pcs') ===
      normalizeLookupKey(nextItem?.unit || 'pcs') &&
...
const useBillingItemHandlers = ({
  ...
  const getCurrentItemValidationError = useCallback(
    (item) => {
  ...
  const handleStartCustomItem = useCallback(
    (rawName = '') => {
  ...
  const handleCommitCurrentItem = useCallback(
    (options = {}) => {
  ...
  const handleDeleteBillItem = useCallback(
    (index) => {
    ...
    (event) => {
      const handleSearchEnter = () => {
        ...
        const exactCodeMatch =
          exactMatch &&
          [normalizeLookupKey(exactMatch?.sku), normalizeLookupKey(exactMatch?.barcode)].includes(
            query
          );
      ...
      if (event.key === 'ArrowDown') {
        ...
        setActiveProductSuggestionIndex((prev) =>
          Math.min(prev + 1, productSearchResults.length - 1)
        );
        return;
      }
  ...
  const handleEntryFieldKeyDown = useCallback(
    (field) => (event) => {
      if (event.ctrlKey && event.key === 'Enter') {
        event.preventDefault();
        if (entryActionLocked) return;
