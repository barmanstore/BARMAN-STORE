...
const getOrderDetailSortValue = (row, columnKey) => {
  ...
  switch (columnKey) {
    case SORTABLE_ORDER_DETAIL_COLUMNS.product:
      return String(item?.product_name || item?.name || item?.product_query || '')
        .trim()
        .toLowerCase();
...
const OrderDetailModal = ({
          ...
          <button
            ...
            onClick={() =>
              setSortConfig((current) =>
                getNextOrderDetailSortConfig(current, SORTABLE_ORDER_DETAIL_COLUMNS.product)
              )
            }
            title={sortLabel}
    ...
    return (
      <th className="sortable" aria-sort={ariaSort}>
        <button
          ...
          onClick={() =>
            setSortConfig((current) => getNextOrderDetailSortConfig(current, columnKey))
          }
          title={sortLabel}
          aria-label={sortLabel}
        >
  ...
  const reviewRows = orderDetailRows.map(({ item, line, originalIndex }) => {
    return {
      ...
      name: String(
        item?.product_name || item?.name || item?.product_query || `Row ${originalIndex + 1}`
      ).trim(),
          ...
          <div className="po-invoice-preview">
            <div className="po-invoice-header">
              ...
              <div className="po-invoice-meta">
                <div>
                  <span>PO Status</span>
                  <strong>{String(getPoLifecycleStatus(orderDetail) || '-').toUpperCase()}</strong>
                </div>
                <div>
                  <span>Payment</span>
                  <strong>{String(getPoPaymentStatus(orderDetail) || '-').toUpperCase()}</strong>
                </div>
                <div>
                  <span>Paid</span>
                  <strong>{formatCurrency(getPoPaidAmount(orderDetail))}</strong>
                </div>
                <div>
                  <span>Balance</span>
                  <strong>{formatCurrency(getPoBalanceDue(orderDetail))}</strong>
                </div>
                <div>
                  <span>Created</span>
                  <strong>
                    {formatDateTime(orderDetail.created_at || orderDetail.order_date)}
                  </strong>
                </div>
                <div>
                  <span>Expected</span>
                  {orderDetailEditMode ? (
                    <input
                      ...
                      onChange={(event) =>
                        handleOrderDetailFieldChange('expected_delivery', event.target.value)
                      }
                    />
                  ) : (
                ...
                <div>
                  ...
                  <strong>{formatDate(orderDetail.payment_due_date)}</strong>
                </div>
                <div>
                  ...
                  {orderDetailEditMode ? (
                    <input
                      ...
                      onChange={(event) =>
                        handleOrderDetailFieldChange('strict_due_date', event.target.value)
                      }
                    />
                  ) : (
                    <strong>
                      {orderDetail.strict_due_date ? formatDate(orderDetail.strict_due_date) : '-'}
                    </strong>
                  )}
                </div>
                <div>
                  <span>Next Action</span>
                  <strong>{getPoNextAction(orderDetail)}</strong>
                </div>
                <div>
                  <span>Bill No</span>
                ...
                {orderDetailEditMode ? (
                  <div className="po-detail-edit-stack">
                    <textarea
                      ...
                      onChange={(event) =>
                        handleOrderDetailFieldChange('notes', event.target.value)
                      }
                    ...
                    <textarea
                      ...
                      onChange={(event) =>
                        handleOrderDetailFieldChange('strict_due_note', event.target.value)
                      }
            ...
            <table className="po-invoice-table">
              ...
              <thead>
                <tr>
                  ...
                  {renderOrderDetailSortableHeader(
                    'Taxable',
                    SORTABLE_ORDER_DETAIL_COLUMNS.taxable
                  )}
              ...
              <tbody>
                {displayedOrderDetailRows.length ? (
                  displayedOrderDetailRows.map(
                    ({ item, originalIndex, line, originalItem }, displayIndex) => {
                      const originalLine = originalItem ? getItemFinancials(originalItem) : null;
                      const rowDiagnostics =
                        orderDetailDraftDiagnostics?.rowDiagnostics?.[originalIndex] || {};
                      const rowChanged =
                        orderDetailEditMode && hasOrderDetailItemChanged(item, originalIndex);
                      const productChanged =
                        orderDetailEditMode &&
                        getOrderDetailItemFieldChanged(item, originalIndex, 'product_id');
                      const qtyChanged =
                        orderDetailEditMode &&
                        getOrderDetailItemFieldChanged(item, originalIndex, 'quantity');
                      const uomChanged =
                        orderDetailEditMode &&
                        getOrderDetailItemFieldChanged(item, originalIndex, 'uom');
                      const rateChanged =
                        orderDetailEditMode &&
                        getOrderDetailItemFieldChanged(item, originalIndex, 'rate');
                      const discountValueChanged =
                        orderDetailEditMode &&
                        getOrderDetailItemFieldChanged(item, originalIndex, 'discount_value');
                      const gstChanged =
                        orderDetailEditMode &&
                        getOrderDetailItemFieldChanged(item, originalIndex, 'gst_rate');
                      const selectedProduct =
                        products.find(
                          (product) => String(product?.id || '') === String(item.product_id || '')
                        ) || null;
                      const uomOptions = getAllowedPurchaseUnitsForProduct(selectedProduct);
                      const quantityStepRaw = getPurchasePackStep(
                        selectedProduct,
                        item.uom || line.uom
                      );
                      const quantityStep =
                        Number.isFinite(quantityStepRaw) && Number.isInteger(quantityStepRaw)
                          ? quantityStepRaw
                          : 1;
                      return (
                        <tr
                          key={item.id || `detail-row-${originalIndex}`}
                          className={rowChanged ? 'po-detail-row-edited' : ''}
                        >
                              ...
                              <>
                                ...
                                <datalist id={`po-detail-product-list-${originalIndex}`}>
                                  {products.map((product) => (
                                    <option
                                      key={`po-detail-product-${originalIndex}-${product.id}`}
                              ...
                              <>
                                ...
                                >
                                  {uomOptions.map((uomOption) => (
                                    <option
                                      key={`detail-item-${originalIndex}-uom-${uomOption}`}
                                      value={uomOption}
                                    >
                                      {uomOption}
                                    </option>
                              ...
                              <>
                                ...
                                >
                                  {GST_RATE_OPTIONS.map((rate) => (
                                    <option
                                      key={`detail-item-${originalIndex}-gst-${rate}`}
                                      value={rate}
                                    >
                                      {rate}%
                                    </option>
                          ...
                          <td
                            className={
                              rowChanged &&
                              (!originalLine ||
                                Math.abs(line.taxableValue - originalLine.taxableValue) > 0.0001)
                                ? 'po-detail-computed-change'
                                : ''
                            }
                          >
                          ...
                          <td
                            className={
                              rowChanged &&
                              (!originalLine ||
                                Math.abs(line.taxAmount - originalLine.taxAmount) > 0.0001)
                                ? 'po-detail-computed-change'
                                : ''
                            }
                          >
                          ...
                          <td
                            className={
                              rowChanged &&
                              (!originalLine ||
                                Math.abs(line.lineTotal - originalLine.lineTotal) > 0.0001)
                                ? 'po-detail-computed-change'
                                : ''
                            }
                          >
                          ...
                          {orderDetailEditMode ? (
                            <td>
                              <div className="po-popup-grid-total-actions">
                                {renderOrderDetailRowConfirmButton(
                                  rowDiagnostics,
                                  originalIndex,
                                  displayIndex
                                )}
                  ...
                  <tr className="po-detail-empty-row">
                    <td colSpan={orderDetailEditMode ? 11 : 10}>
                      No rows match the product search.
                    </td>
                  </tr>
                )}
              </tbody>
            ...
            <div className="po-invoice-summary">
              <div
                className={`po-summary-row${orderDetailHasComputedChanges ? ' po-detail-computed-change' : ''}`}
              >
              ...
              <div
                className={`po-summary-row grand${orderDetailHasComputedChanges ? ' po-detail-computed-change' : ''}`}
              >
        ...
        <div className="order-detail-body">
          <PurchaseOrderReviewSheet
            ...
            metaItems={[
              {
                label: 'Supplier:',
                value: String(orderDetailSupplier?.name || '').trim() || 'Not selected',
              },
              {
                label: 'Date:',
                value: formatDate(orderDetail?.order_date || orderDetail?.created_at),
              },
              {
                label: 'Delivery:',
                value: orderDetail?.expected_delivery
                  ? formatDate(orderDetail.expected_delivery)
                  : 'Skipped',
              },
            ]}
            rows={reviewRows}
        ...
        {orderDetailIsEditable ? (
          <button
            ...
            disabled={
              ...
              orderDetailEditMode ||
              orderDetailSaving ||
              sendingWhatsAppOrderId === orderDetail?.id
            }
          >
