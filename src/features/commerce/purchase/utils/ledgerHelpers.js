...
export const getRecordDate = (entry) =>
  getLedgerEntryTimestamp(entry, ['transaction_date', 'created_at', 'date']);
...
export const getEntryTypeKey = (entry) =>
  String(entry?.type || entry?.transaction_type || '')
    .trim()
    .toLowerCase();
...
export const getLedgerDistributorKey = (entry) => {
  if (entry?.distributor_id !== undefined && entry?.distributor_id !== null)
    return String(entry.distributor_id);
...
export const getEntryDedupKey = (entry) => {
  ...
  if (reference || dateKey)
    return `fallback:${distributorKey}|${type}|${reference}|${amount}|${dateKey}`;
...
export const getDerivedLedgerFromOrders = (orders, selectedDistributorId) => {
  ...
  const derived = (orders || [])
    ...
    .map((order) => ({
      ...
      transaction_date:
        order.created_at || order.order_date || order.expected_delivery || getTodayDate(),
...
export const mergeLedgerRecords = (apiRecords, localRecords, derivedRecords) => {
  ...
  const existingSourceKeys = new Set(
    [...baseRecords, ...local].map((entry) => getEntrySourceKey(entry)).filter(Boolean)
  );
    ...
    if (poIdentityKey) {
      const duplicatePoEntry = [...baseRecords, ...local].some((existingEntry) => {
        ...
        const sameDistributor =
          getLedgerDistributorKey(existingEntry) === getLedgerDistributorKey(entry);
        if (!sameDistributor) return false;

        const sameReference =
          normalizeTextKey(existingEntry?.reference || existingEntry?.po_number) ===
          normalizeTextKey(entry?.reference || entry?.po_number);
...
export const getLedgerBalanceSummary = (records, selectedDistributorId) => {
  ...
  const totalBalance = Object.values(balancesByDistributor).reduce(
