...
function usePurchaseCalculations({
  ...
  const buildOrderDraftItem = useCallback(
    (product = null, overrides = {}) => {
      const resolvedProduct = product || null;
      const resolvedRate = Math.max(
        0,
        toNumber(overrides.rate ?? overrides.unit_price ?? resolvedProduct?.price)
      );
      ...
      return {
        ...
        product_id: resolvedProduct?.id
          ...
          : String(overrides.product_id || ''),
        product_query: resolvedProduct
          ? getProductSearchLabel(resolvedProduct)
          : String(overrides.product_query || overrides.product_name || '').trim(),
        ...
        rate: resolvedRate,
        rate_warning_acknowledged: Boolean(overrides.rate_warning_acknowledged),
        reference_rate: Math.max(0, toNumber(overrides.reference_rate ?? resolvedRate)),
        reference_rate_source: String(
          overrides.reference_rate_source || (resolvedRate > 0 ? 'current reference rate' : '')
        ).trim(),
        ...
        discount_warning_acknowledged: Boolean(overrides.discount_warning_acknowledged),
        last_purchase_hint: String(overrides.last_purchase_hint || '').trim(),
        last_purchase_rate: Math.max(0, toNumber(overrides.last_purchase_rate)),
        last_purchase_distributor_name: String(
          overrides.last_purchase_distributor_name || ''
        ).trim(),
        last_purchase_created_at: String(overrides.last_purchase_created_at || '').trim(),
        last_purchase_po_number: String(overrides.last_purchase_po_number || '').trim(),
        auto_fill_seed_rate: overrides.auto_fill_seed_rate ?? null,
        auto_fill_seed_gst_rate: overrides.auto_fill_seed_gst_rate ?? null,
        auto_fill_seed_uom: String(overrides.auto_fill_seed_uom || '').trim(),
      };
    },
    [createEmptyOrderItem, normalizeGstRateOption, resolvePurchaseUnitForProduct, toNumber]
  );

  const calculateOrderItem = useCallback(
    (item) => {
      ...
      const discountAmountRaw =
        discountType === 'percent' ? (grossAmount * discountValue) / 100 : discountValue;
