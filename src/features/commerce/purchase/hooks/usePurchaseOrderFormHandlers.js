...
import { findActivePurchaseProduct, isActivePurchaseProduct } from '../utils/productSearch';
import {
...
const usePurchaseOrderFormHandlers = ({
  ...
  const buildSuggestedProductSnapshot = useCallback(
    (item = {}) => {
      const rawSnapshot =
      ...
      return {
        ...
        price: toNumber(
          rawSnapshot?.price ?? item?.reference_rate ?? item?.unit_price ?? item?.rate ?? 0
        ),
        uom: String(rawSnapshot?.uom || item?.uom || 'pcs').trim() || 'pcs',
        base_unit:
          String(rawSnapshot?.base_unit || item?.base_unit || item?.uom || 'pcs').trim() || 'pcs',
        conversion_factor:
          toNumber(rawSnapshot?.conversion_factor ?? item?.conversion_factor ?? 1) || 1,
        purchase_pack_size: toNumber(
          rawSnapshot?.purchase_pack_size ?? item?.purchase_pack_size ?? 0
        ),
  ...
  const resolveSuggestedProduct = useCallback(
    (item = {}) => {
      const rawSnapshot =
        item?.product_snapshot && typeof item.product_snapshot === 'object'
          ? item.product_snapshot
      ...
      const rawProduct = productId
        ? (Array.isArray(products) ? products : []).find(
            (entry) => String(entry?.id || '').trim() === productId
          ) || null
        : null;
      if (rawProduct && !isActivePurchaseProduct(rawProduct)) return null;
      if (snapshot && (snapshot.is_active === false || Number(snapshot.is_active || 1) === 0))
        return null;
      return snapshot;
    },
  ...
  const resolveSuggestedItemHistorySuggestion = useCallback(
  ...
  const buildSuggestedOrderItems = useCallback(
    (suggestedItems = [], distributorName = '', fallbackDate = '', distributorId = '') =>
      suggestedItems.length
        ? suggestedItems
            .map((item) => {
              ...
              const historySuggestion = resolveSuggestedItemHistorySuggestion(
                item.product_id,
                distributorId
              );
  ...
  const resolvePrimarySupplier = useCallback(
    (distributorId) => {
      const normalizedId = String(distributorId || '').trim();
      if (!normalizedId) return null;
      const matches = (Array.isArray(suppliers) ? suppliers : []).filter(
        (entry) => String(entry?.distributor_id || '') === normalizedId
      );
  ...
  const hydrateSuggestedOrderItemsFromLastPurchase = useCallback(
    (suggestedItems = [], distributorId = '') => {
        ...
        Promise.resolve(loadLastPurchaseSuggestion(selectedProductId))
          .then((suggestion) => {
            ...
            setOrderFormData((prev) => {
              ...
              if (!current || String(current.product_id || '').trim() !== selectedProductId)
                return prev;
              ...
              nextItems[index] = applyPurchaseDraftLastPurchaseSuggestion({
                ...
                preserveQuantity: true,
                preserveRate:
                  preserveFlags.preserveRate || item.rate != null || item.unit_price != null,
  ...
  const openCreateOrderForm = useCallback(
    (options = {}) => {
      ...
      const suggestedItems = Array.isArray(safeOptions.suggested_items)
        ? safeOptions.suggested_items
        : [];
      ...
      setOrderFormData({
        ...
        items: buildSuggestedOrderItems(suggestedItems, '', plannedOrderDate, ''),
      });
  ...
  const openCreateOrderFormForDistributor = useCallback(
    (distributorId, options = {}) => {
      ...
      const resolvedDistributorId = distributor
        ? String(distributor.id)
        : String(distributorId || '');
      const requestedSupplierId = String(
        safeOptions.supplier_id ?? safeOptions.supplierId ?? ''
      ).trim();
      const supplierMatches = (Array.isArray(suppliers) ? suppliers : []).filter(
        (entry) => String(entry?.distributor_id || '') === resolvedDistributorId
      );
      const supplier = requestedSupplierId
        ? supplierMatches.find(
            (entry) => String(entry?.id || '') === requestedSupplierId && entry?.is_active !== false
          ) ||
  ...
  const handlePurchaseSectionChange = useCallback(
    (sectionKey) => {
  ...
  const handleOpenOrderReview = useCallback(() => {
    ...
    const selectedSupplier = (Array.isArray(suppliers) ? suppliers : []).find(
      (s) => String(s.id) === String(orderFormData.supplier_id) && s.is_active !== false
    );
    const selectedDistributor = distributors.find(
      (d) => String(d.id) === String(orderFormData.distributor_id) && d.status === 'active'
    );
    if (
      !selectedSupplier ||
      !selectedDistributor ||
      String(selectedSupplier.distributor_id || '') !== String(selectedDistributor.id || '')
    ) {
    ...
    async (e) => {
      ...
      try {
        ...
        const [nextOrders] = await Promise.all([
          fetchOrders(),
          typeof fetchOperationsSummary === 'function'
            ? fetchOperationsSummary()
            : Promise.resolve(),
        ...
        const savedOrder =
          nextOrders.find((order) => String(order?.id || '') === savedOrderId) || null;
        const savedPoNumber = String(response?.po_number || savedOrder?.po_number || '').trim();
        ...
        setLastSavedOrderSummary({
          ...
          totalAmount:
            Number(
              savedOrder?.total_amount ?? savedOrder?.total ?? submission.totals?.totalAmount ?? 0
            ) || 0,
      ...
      try {
        ...
        setOrderFormData({
          ...
          distributor_name:
            ...
            distributors.find((d) => String(d.id) === String(order.distributor_id))?.name ||
            '',
