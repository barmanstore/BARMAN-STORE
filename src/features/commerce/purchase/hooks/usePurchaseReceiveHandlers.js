...
const usePurchaseReceiveHandlers = ({
  ...
  const handleReceiveClick = useCallback((order) => {
  ...
  const handleReceiveItemChange = useCallback((index, field, value) => {
    const items = [...receiveData.items];
    items[index][field] = value;
    setReceiveData((prev) => ({ ...prev, items }));
  }, [receiveData.items, setReceiveData]);

  const handleReceiveQtyStep = useCallback((index, delta) => {
  ...
  const handleReceiveSubmit = useCallback(async (e) => {
    e.preventDefault();
