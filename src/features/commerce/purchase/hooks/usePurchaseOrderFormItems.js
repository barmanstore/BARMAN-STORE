...
const usePurchaseOrderFormItems = ({
  ...
  const handleOrderItemRemove = useCallback(
    (index) => {
      setOrderFormData((prev) => {
        const targetItem = prev.items?.[index];
        const locked =
          targetItem?.po_item_locked === true ||
          targetItem?.po_item_source === 'supplier_default' ||
          String(targetItem?.row_source || '')
            .trim()
  ...
  const ensureOrderFormItemAtIndex = useCallback(
    (index) => {
      setOrderFormData((prev) => {
  ...
  const handleOrderProductFieldFocus = useCallback(
    (index) => {
  ...
  const handleLoadDistributorItems = useCallback(() => {
    const selectedDistributor = distributors.find(
      (entry) =>
        String(entry.id) === String(orderFormData.distributor_id) && entry.status === 'active'
    );
    ...
    try {
      ...
      const nextItems = getDistributorHistoryProducts(selectedDistributor.id).filter(
        (item) => !existingProductIds.has(String(item.product_id || '').trim())
      );
      ...
      setSuccess(
        `Loaded ${nextItems.length} supplier item${nextItems.length === 1 ? '' : 's'} with recent qty and rate.`
      );
    } finally {
  ...
  const handleApplySupplierHistoryItem = useCallback(
    (suggestedItem, overrides = null) => {
      const productId = String(suggestedItem?.product_id || '').trim();
      if (!productId) return;
      const resolvedOverrides =
        overrides && typeof overrides === 'object' ? overrides : { quantity: overrides };
      const resolvedQuantity = Math.max(
        1,
        Number(resolvedOverrides?.quantity ?? suggestedItem?.quantity ?? 1) || 1
      );
      const resolvedUom =
        String(resolvedOverrides?.uom || suggestedItem?.uom || 'pcs').trim() || 'pcs';
