...
const usePurchasePaymentHandlers = ({
  ...
  const resolveLatestPaymentOrder = useCallback(
    async (order) => {
  ...
  const handleOpenPoPaymentModal = useCallback(
    (order) => {
      if (!order) return;
      const openModal = async () => {
        ...
        setPoPaymentFormData({
          ...
          reference: String(
            resolvedOrder.bill_number ||
              resolvedOrder.invoice_number ||
              resolvedOrder.po_number ||
              ''
          ).trim(),
  ...
  const handleOpenPoPaymentById = useCallback(
    (orderId) => {
  ...
  const handlePoPaymentSubmit = useCallback(
    async (e) => {
      e.preventDefault();
