...
} from '../../../../shared/services/invalidation';
import {
...
import { createEmptyOrderItem, findProductForItem } from '../utils/items';
import {
...
import { readSavedPurchaseDrafts, writeSavedPurchaseDrafts } from '../utils/savedDrafts';
...
const usePurchaseManagementController = ({
  ...
  const { poModalSize, poModalRef } = usePoModalSizing({
    isMobile,
  ...
  const { buildOrderDraftItem, calculateOrderItem, calculateOrderTotals } = usePurchaseCalculations(
    {
      products,
      toNumber,
  ...
  const { fetchData, fetchOrders, fetchOperationsSummary, fetchReturns, fetchDistributorLedger } =
  ...
  const patchSupplierVisitSummary = useCallback(
    (supplierId, date, patch = {}) => {
      ...
      setOperationsSummary((current) => {
        ...
        const visits = Array.isArray(baseSummary.supplier_visits)
  ...
  useEffect(
    () =>
      registerDomainListener(DOMAINS.Products, refreshPurchaseData, {
        listenerId: 'purchase-products',
      }),
    [refreshPurchaseData]
  );

  const buildPageProps = () =>
    buildPurchaseManagementPageProps(
      {
        loading,
        error,
        success,
      ...
      },
      {
        ...
        supplierRegisteredProducts: selectedSupplierRegisteredProducts,
        orderTotals,
        getProductSearchOptionLabel,
        orderSubmitting,
        savedOrderDrafts,
  ...
  const handleCloseSupplierVisit = useCallback(
    async ({ supplierId, date } = {}) => {
    ...
    [fetchOperationsSummary, patchSupplierVisitSummary, setError, setSuccess]
  );

  const handleReopenSupplierVisit = useCallback(
    async ({ supplierId, date } = {}) => {
      const normalizedSupplierId = Number(supplierId || 0) || null;
      const visitDate = toDateInputValue(date || getTodayDate());
      if (!normalizedSupplierId || !visitDate) return false;
  ...
  const handleOrderSaved = useCallback(() => {
    ...
    persistSavedOrderDrafts((current) =>
      current.filter((entry) => entry.id !== activeSavedOrderDraftId)
    );
  ...
  const loadSupplierRegisteredProducts = useCallback(
    async (supplierId, options = {}) => {
    ...
    [setError, setLoadingDistributorItems, supplierRegisteredProductsBySupplier]
  );
  const refreshSupplierRegisteredProducts = useCallback(
    async (supplierId) => {
      const normalizedSupplierId = String(supplierId || '').trim();
      if (!normalizedSupplierId) return [];
  ...
  const scheduleLoadSupplierRegisteredProducts = useCallback(
    (supplierId) => {
  ...
  const buildSupplierDefaultOrderItem = useCallback(
    (registeredProduct = null, distributorId = '') => {
      const productId = String(registeredProduct?.product_id || registeredProduct?.id || '').trim();
      if (!productId) return null;

      const product =
        findActivePurchaseProduct(products, productId) ||
        (registeredProduct?.is_active === false || Number(registeredProduct?.is_active ?? 1) === 0
          ? null
          : registeredProduct) ||
        null;
      ...
      let nextItem = buildOrderDraftItem(product, {
        ...
        product_name:
          registeredProduct?.name || registeredProduct?.product_name || product?.name || '',
        quantity: 0,
        uom:
          registeredProduct?.base_unit ||
          registeredProduct?.uom ||
          product?.base_unit ||
          product?.uom ||
          'pcs',
        rate: fallbackRate,
    ...
    [buildOrderDraftItem, products, purchaseOrders]
  );

  const buildSupplierBoardItems = useCallback(
    ({ distributorId = '', currentItems = [], registeredProducts = [] }) => {
      ...
      (Array.isArray(registeredProducts) ? registeredProducts : []).forEach((registeredProduct) => {
        if (
          registeredProduct?.is_active === false ||
          Number(registeredProduct?.is_active ?? 1) === 0
        )
          return;
        const productId = String(
          registeredProduct?.product_id || registeredProduct?.id || ''
        ).trim();
        ...
        const seededItem = buildSupplierDefaultOrderItem(
          registeredProduct,
  ...
  const handleShortcutDraftClosed = useCallback(
    (reason = 'cancel') => {
      ...
      onShortcutDraftClosed({
        ...
        reason,
        returnTab:
          String(activeContext?.returnTab || 'restock-dashboard').trim() || 'restock-dashboard',
      });
    },
  ...
  const openCreateOrderFormForDistributor = useCallback(
    (distributorId, options = {}, context = null) => {
      shortcutDraftHadMeaningfulItemsRef.current = false;
      setShortcutDraftContext(context && typeof context === 'object' ? context : null);
      setActiveSavedOrderDraftId('');
      activeSavedOrderDraftIdRef.current = '';
  ...
  const getShortcutAction = useCallback((payload = null) => {
    const action = String(payload?.action || 'create-draft')
      .trim()
      .toLowerCase();
  ...
  const applyShortcutPayload = useCallback(
    (payload = null, shortcutContext = null) => {
  ...
  const normalizeRestoredOrderForm = useCallback((value) => {
    ...
    const plannedOrderDate =
      String(restoredOrderForm?.planned_order_date || '').trim() || getTodayDate();
    return {
      ...restoredOrderForm,
      planned_order_date: plannedOrderDate,
    };
  }, []);
  const hasMeaningfulOrderItems =
    Array.isArray(orderFormData?.items) &&
    orderFormData.items.some(
      (item) =>
  ...
  const hasOpenDirtyOrderDraft = Boolean(
    ...
    (editingOrderId ||
      ...
      String(orderFormData?.planned_order_date || '').trim() !==
        String(defaultOrderFormData.planned_order_date || '').trim() ||
      String(orderFormData?.expected_delivery || '').trim() !==
        String(defaultOrderFormData.expected_delivery || '').trim() ||
  ...
  useEffect(() => {
    ...
    const matchedDistributor = distributors.find(
      (entry) => String(entry?.id || '') === distributorId
    );
  ...
  useEffect(() => {
    ...
    const normalizeDistributorName = (value) =>
      String(value || '')
        .toLowerCase()
        .replace(/\s+/g, ' ')
        .trim();
    ...
    const exactMatch = distributors.find(
      (entry) =>
        isActiveDistributor(entry) && normalizeDistributorName(entry?.name) === normalizedName
    );
    const startsWithMatches = distributors.filter(
      (entry) =>
        isActiveDistributor(entry) &&
        normalizeDistributorName(entry?.name).startsWith(normalizedName)
    );
    const containsMatches = distributors.filter(
      (entry) =>
        isActiveDistributor(entry) && normalizeDistributorName(entry?.name).includes(normalizedName)
    );
    const matchedDistributor =
      exactMatch ||
  ...
  useEffect(() => {
    ...
    setOrderFormData((prev) => {
      ...
      if (
        ...
        String(prev?.distributor_id || '').trim() !== supplierDistributorId
      ) {
  ...
  useEffect(() => {
    ...
    setOrderFormData((prev) => {
      ...
      restockPendingSelection.items.forEach((row) => {
        ...
        if (!product) return;
        const fallbackRate =
          Number(row?.rate ?? row?.unit_price ?? productSnapshot?.price ?? product?.price ?? 0) ||
          0;
  ...
  useEffect(() => {
    if (
      !showOrderForm ||
      editingOrderId ||
      shortcutDraftContext?.source !== 'restock' ||
      orderReviewMode
    ) {
      shortcutDraftHadMeaningfulItemsRef.current = false;
      return;
  ...
  useEffect(() => {
    ...
    const shortcutPayload =
      shortcutOpenOrderPayload && typeof shortcutOpenOrderPayload === 'object'
  ...
  usePopupDraftPersistence({
    ...
    onRestore: (draft) => {
      const restoredOrderForm =
        draft?.orderFormData && typeof draft.orderFormData === 'object'
      ...
      setOrderFormData({
        ...
        ...restoredOrderForm,
        items:
          Array.isArray(restoredOrderForm.items) && restoredOrderForm.items.length
  ...
  useEffect(() => {
    ...
    setActiveSubTab('orders');
    openCreateOrderForm();
  }, [autoOpenOrderForm, openCreateOrderForm, popupMode, setActiveSubTab, showOrderForm]);
  ...
  const saveCurrentOrderDraft = useCallback(() => {
    ...
    const now = Date.now();
    const nextDraftId =
      String(activeSavedOrderDraftIdRef.current || activeSavedOrderDraftId || '').trim() ||
      `purchase-draft-${now}`;
    const supplierName = String(
      orderFormData?.supplier_name || orderFormData?.distributor_name || ''
    ).trim();
    const meaningfulItemCount = orderDraftProjection.rows.filter(
      (row) => Number(row?.item?.product_id || 0) > 0
    ).length;
    const draftTitle = supplierName
      ? `${supplierName} draft`
      : `Purchase draft ${new Date(now).toLocaleString()}`;
    const cleanItems = (Array.isArray(orderFormData?.items) ? orderFormData.items : []).map(
      (item) => {
    ...
    const nextDraftEntry = {
      ...
      orderFormData: {
        ...restOrderFormData,
        items: cleanItems.length ? cleanItems : [createEmptyOrderItem()],
      },
    };
  ...
  const openSavedOrderDraft = useCallback(
    (draftId) => {
      ...
      const restoredOrderForm =
        selectedDraft.orderFormData && typeof selectedDraft.orderFormData === 'object'
  ...
  const deleteSavedOrderDraft = useCallback(
    (draftId) => {
    ...
    [activeSavedOrderDraftId, persistSavedOrderDrafts, savedOrderDrafts, setSuccess]
  );
