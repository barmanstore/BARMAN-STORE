...
const usePurchaseOrderDetailHandlers = ({
  ...
  const buildOrderDetailDraft = useCallback(
    (order) => {
      ...
      return {
        ...
        items: (order.items || []).map((item) => {
          ...
          return {
            ...
            reference_rate: Math.max(
              0,
              toNumber(item.reference_rate ?? item.rate ?? item.unit_price)
            ),
  ...
  const handleOrderDetailFieldChange = useCallback(
    (field, value) => {
  ...
  const handleOrderDetailItemChange = useCallback(
    (index, field, value) => {
  ...
  const handleOrderDetailProductInputChange = useCallback(
    async (index, value) => {
  ...
  const hydrateOrderDetailDraftSuggestions = useCallback(
    async (draft) => {
  ...
  const handleOrderDetailItemRemove = useCallback(
    (index) => {
      setOrderDetailDraft((prev) => {
        if (!prev) return prev;
        return {
  ...
  const handleOrderDetailSave = useCallback(async () => {
    ...
    try {
      ...
      const payload = buildPurchaseOrderSavePayload({
        ...
        plannedOrderDate:
          orderDetailDraft.planned_order_date || orderDetail.planned_order_date || null,
