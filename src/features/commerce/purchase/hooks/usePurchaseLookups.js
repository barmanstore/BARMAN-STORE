...
const usePurchaseLookups = ({
  ...
  const resolveSupplierByInput = useCallback(
    (value) => {
      ...
      const distributorSuppliers = (Array.isArray(suppliers) ? suppliers : []).filter((entry) => {
        const isActive = entry?.is_active !== false;
        return (
          isActive && String(entry?.distributor_id || '') === String(distributorMatch.id || '')
        );
      });
  ...
  const resolveProductByInput = useCallback(
    (value) => resolveProductByInputHelper(value, products),
    [resolveProductByInputHelper, products]
  );

  const getDistributorProductOptions = useCallback(
    (distributorId) =>
      getDistributorProductOptionsHelper({
        distributorId,
        products,
  ...
  const getDistributorProductHistoryEntry = useCallback(
  ...
  const getLatestProductHistoryEntry = useCallback(
    (productId) =>
      getLatestProductHistoryEntryHelper({
        productId,
        products,
  ...
  const getDistributorHistoryProducts = useCallback(
    (distributorId) =>
      getDistributorHistoryProductsHelper({
        distributorId,
        products,
  ...
  const handleDistributorInputChange = useCallback(
    (value) => {
      ...
      setOrderFormData((prev) => {
        ...
        return {
          ...
          items: distributorChanged ? [createEmptyOrderItem()] : prev.items,
        };
      });
    },
