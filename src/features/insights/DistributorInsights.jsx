...
import {
  ...
  FilterRow,
  FilterTray,
} from '../../shared/components/filters/FilterSurface';
...
const DistributorInsights = () => {
  ...
  const activeFilterPills = useMemo(() => {
    ...
    if (filters.start_date || filters.end_date) {
      const label = activeDatePresetLabel
    ...
    return pills;
  }, [activeDatePresetLabel, filters.end_date, filters.start_date]);
  ...
  return (
    <div className="insights-page distributor-insights product-insights">
      ...
      <FilterBar className="distributor-insights-filters">
        <SearchFilter
      ...
      <FilterPills
        items={activeFilterPills.map((pill) => ({
          ...pill,
          tone: pill.key === 'search' ? 'sky' : 'violet',
          icon: <X size={12} aria-hidden="true" />,
        }))}
      />

      {showAdvancedFilters ? (
        <FilterTray ref={advancedFiltersRef} id="distributor-insights-advanced-filters">
      ...
      <div className="summary-stats">
        <StatCard label="Total Distributors" value={summaryStats.totalDistributors} tone="sky" />
        <StatCard label="High Risk" value={summaryStats.highRiskCount} tone="amber" />
        <StatCard label="Reliable" value={summaryStats.reliableCount} tone="emerald" />
        <StatCard label="Wide Catalog" value={summaryStats.wideCatalogCount} tone="violet" />
      </div>
