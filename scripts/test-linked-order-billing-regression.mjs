...
const main = async () => {
  ...
  try {
    ...
    if (!adminId) {
      const createdAdmin = await pool.query(
        `INSERT INTO users (role, name, email, email_verified, phone, phone_verified, address, password_hash, must_change_password)
         ...
         RETURNING id`,
        ['admin', 'Linked Smoke Admin', `admin-smoke-${randomSuffix()}@example.com`, 1, null, 0, null, 'smoke-hash', 0]
      );
    ...
    assert.equal(Number(orderCreateJson?.totalAmount || 0), 297, 'order total should include first-order discount before tax');
    ...
    assert.equal(Number(firstOrderItem?.line_subtotal || 0), 300, 'order line subtotal should preserve pre-discount amount');
    assert.equal(Number(firstOrderItem?.offer_discount || 0), 30, 'order line should persist original offer discount');
    assert.equal(Number(firstOrderItem?.manual_discount || 0), 0, 'order line should persist manual discount separately');
    assert.equal(String(firstOrderItem?.offer_label || '').trim(), '10% OFF | First order only', 'order line should persist original offer label');
    assert.equal(Number(firstOrderItem?.total || 0), 270, 'order line total should reflect the original discount');
    ...
    assert.equal(offerUpdateRes.status, 200, `offer update failed: ${JSON.stringify(offerUpdateJson)}`);
    assert.equal(Number(offerUpdateJson?.value || 0), 25, 'offer value should update after order placement');

    const receiveRes = await adminRequest(`/api/orders/${orderId}/status`, {
      method: 'PUT',
      body: JSON.stringify({ status: 'received', description: 'Linked billing regression receive' }),
    });
    const receiveJson = await toJson(receiveRes);
    assert.equal(receiveRes.status, 200, `receive failed: ${JSON.stringify(receiveJson)}`);
    assert.equal(Number(receiveJson?.fulfilled_qty || 0), 3, 'received order should fulfill all requested quantity');
    assert.equal(Number(receiveJson?.pending_qty || 0), 0, 'received order should have no pending quantity');
    ...
    assert.equal(invalidLinkedBillRes.status, 400, `linked bill should reject invalid order item ids: ${JSON.stringify(invalidLinkedBillJson)}`);
    ...
    assert.equal(billCreateRes.status, 201, `linked bill create failed: ${JSON.stringify(billCreateJson)}`);
    assert.equal(Boolean(billCreateJson?.stock_applied), false, 'linked order billing should not apply stock twice');
    ...
    assert.equal(persistedBillRes.status, 200, `linked bill fetch failed: ${JSON.stringify(persistedBillJson)}`);
    assert.equal(Number(persistedBillJson?.subtotal || 0), 300, 'linked bill subtotal should preserve original order subtotal');
    assert.equal(Number(persistedBillJson?.discount_amount || 0), 30, 'linked bill discount should preserve original order discount');
    assert.equal(Number(persistedBillJson?.total_amount || 0), 270, 'linked bill total should preserve original order total');
    const persistedBillItem = Array.isArray(persistedBillJson?.items) ? persistedBillJson.items[0] : null;
    assert.equal(Number(persistedBillItem?.line_subtotal || 0), 300, 'linked bill item should preserve original subtotal');
    assert.equal(Number(persistedBillItem?.offer_discount || 0), 30, 'linked bill item should preserve original offer discount');
    assert.equal(Number(persistedBillItem?.manual_discount || 0), 0, 'linked bill item should preserve original manual discount');
    assert.equal(String(persistedBillItem?.offer_label || '').trim(), '10% OFF | First order only', 'linked bill item should preserve original offer label');
    assert.equal(Number(persistedBillItem?.amount || 0), 270, 'linked bill item should preserve original discounted amount');
    ...
    assert.equal(duplicateBillRes.status, 200, `duplicate linked bill should dedupe: ${JSON.stringify(duplicateBillJson)}`);
    assert.equal(Boolean(duplicateBillJson?.deduplicated), true, 'duplicate linked bill should be deduplicated');
    ...
    assert.equal(restockRes.status, 200, `restock before partial bill failed: ${JSON.stringify(restockJson)}`);
    assert.equal(Number(restockJson?.stock || 0), 1, 'restock should prepare one unit for partial linked billing');
    ...
    assert.equal(partialOrderRes.status, 201, `partial linked order create failed: ${JSON.stringify(partialOrderJson)}`);
    const partialOrderId = Number(partialOrderJson?.orderId || 0);
    assert.equal(partialOrderId > 0, true, 'partial linked order id missing');
    assert.equal(Number(partialOrderJson?.totalAmount || 0), 220, 'second order should not reuse first-order-only discount');
    ...
    assert.equal(partialReceiveRes.status, 200, `partial receive failed: ${JSON.stringify(partialReceiveJson)}`);
    assert.equal(Number(partialReceiveJson?.fulfilled_qty || 0), 1, 'partial receive should only fulfill available quantity');
    assert.equal(Number(partialReceiveJson?.pending_qty || 0), 1, 'partial receive should keep the remaining quantity pending');
    ...
    assert.equal(partialBillRes.status, 201, `partial linked bill create failed: ${JSON.stringify(partialBillJson)}`);
    assert.equal(Number(partialBillJson?.fulfilled_qty || 0), 1, 'partial linked bill response should report one billed unit');
    assert.equal(Number(partialBillJson?.pending_qty || 0), 1, 'partial linked bill response should report one pending unit');
    ...
    assert.equal(partialPersistedBillRes.status, 200, `partial linked bill fetch failed: ${JSON.stringify(partialPersistedBillJson)}`);
    assert.equal(Number(partialPersistedBillJson?.subtotal || 0), 100, 'partial linked bill subtotal should bill only fulfilled quantity');
    assert.equal(Number(partialPersistedBillJson?.discount_amount || 0), 0, 'partial linked bill should not invent discounts');
    assert.equal(Number(partialPersistedBillJson?.total_amount || 0), 100, 'partial linked bill total should bill only fulfilled quantity');
    const partialPersistedBillItem = Array.isArray(partialPersistedBillJson?.items) ? partialPersistedBillJson.items[0] : null;
    assert.equal(Number(partialPersistedBillItem?.qty || 0), 1, 'partial linked bill item qty should match fulfilled quantity');
    assert.equal(Number(partialPersistedBillItem?.requested_qty || 0), 2, 'partial linked bill item should keep requested quantity');
    assert.equal(Number(partialPersistedBillItem?.fulfilled_qty || 0), 1, 'partial linked bill item should persist fulfilled quantity');
    assert.equal(Number(partialPersistedBillItem?.pending_qty || 0), 1, 'partial linked bill item should persist pending quantity');
    assert.equal(Number(partialPersistedBillItem?.line_subtotal || 0), 100, 'partial linked bill item subtotal should bill only fulfilled quantity');
    assert.equal(Number(partialPersistedBillItem?.amount || 0), 100, 'partial linked bill item amount should bill only fulfilled quantity');
