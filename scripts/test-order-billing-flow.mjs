...
const main = async () => {
  ...
  try {
    ...
    if (!hasDbEnv) {
      throw new Error(
        'Order flow smoke test requires SMOKE_TEST_DB_URL. Set SMOKE_TEST_ALLOW_PRIMARY_DB=1 only if you intentionally want to reuse the primary app DB.'
      );
    }
    ...
    if (!ready) {
      ...
      if (allowSkipIfNoDb && dbBootFailed) {
        logSmokeSkipInfo('Order flow smoke test skipped because the database is unavailable.', [
          'Set SMOKE_TEST_DB_URL to run the test safely.',
        ]);
        return;
      }
    ...
    assert.equal(
      futureOfferRes.status,
      201,
      `future offer create failed: ${JSON.stringify(futureOfferJson)}`
    );
    ...
    assert.equal(
      allScopeOfferRes.status,
      201,
      `all-scope offer create failed: ${JSON.stringify(allScopeOfferJson)}`
    );
    ...
    const productsListRes = await request(
      `/api/products?page=1&page_size=12&name=${encodeURIComponent(productName)}`
    );
    const productsListJson = await toJson(productsListRes);
    assert.equal(
      productsListRes.status,
      200,
      `products list failed: ${JSON.stringify(productsListJson)}`
    );
    assert.match(
    ...
    assert.equal(
      Boolean(listedProduct?.offer_display?.has_offer),
      true,
      'products list should decorate product with active offers'
    );
    assert.equal(
      Number(listedProduct?.offer_display?.display_price || 0),
      90,
      'products list should expose discounted display price'
    );
    assert.equal(
    ...
    assert.equal(
      Array.isArray(listedProduct?.active_offer_labels) &&
        listedProduct.active_offer_labels.includes('10% OFF | First order only'),
      true,
      'products list should expose active offer badges'
    );
    assert.equal(
      Array.isArray(listedProduct?.active_offer_labels) &&
        listedProduct.active_offer_labels.includes('Save Rs 7 | First order only'),
      true,
      'products list should expose ALL-scoped offer badges'
    );
    assert.equal(
      Array.isArray(listedProduct?.active_offer_labels) &&
        listedProduct.active_offer_labels.includes('5% OFF | First order only'),
      true,
    ...
    assert.equal(
      productDetailWithOfferRes.status,
      200,
      `product detail with offer failed: ${JSON.stringify(productDetailWithOfferJson)}`
    );
    assert.equal(
      Boolean(productDetailWithOfferJson?.offer_display?.has_offer),
      true,
      'product detail should include offer decoration'
    );
    assert.equal(
    ...
    assert.equal(
      Boolean(suggestedProduct?.offer_display?.has_offer),
      true,
      'product suggest should include offer decoration'
    );
    assert.equal(
      Array.isArray(suggestedProduct?.active_offer_labels) &&
        suggestedProduct.active_offer_labels.includes('10% OFF | First order only'),
      true,
    ...
    assert.equal(
      previewBeforeOrderRes.status,
      200,
      `offer preview before order failed: ${JSON.stringify(previewBeforeOrderJson)}`
    );
    ...
    assert.equal(
      Number(previewBeforeOrderJson?.summary?.total || 0),
      297,
      'offer preview total should include discount and tax before first order'
    );
    ...
    assert.equal(
      ...
      297,
      'order total should include offer discount before tax'
    );
    ...
    assert.equal(
      Number(createdOrderItem?.line_subtotal || 0),
      300,
      'order line subtotal should preserve pre-discount amount'
    );
    assert.equal(
      Number(createdOrderItem?.offer_discount || 0),
      30,
      'order item should persist applied offer discount'
    );
    assert.equal(
      Number(createdOrderItem?.manual_discount || 0),
      0,
      'order item should persist manual discount separately'
    );
    assert.equal(
      String(createdOrderItem?.offer_label || '').trim(),
      '10% OFF | First order only',
      'order item should persist applied offer label'
    );
    assert.equal(
      Number(createdOrderItem?.total || 0),
      270,
      'order item total should reflect discount'
    );
    ...
    assert.equal(
      ...
      200,
      `offer update after order create failed: ${JSON.stringify(offerUpdateJson)}`
    );
    assert.equal(
      Number(offerUpdateJson?.value || 0),
      25,
      'offer update should persist the new percentage'
    );
    ...
    assert.equal(
      billBeforeReceiveRes.status,
      409,
      `order-linked bill should block before receive: ${JSON.stringify(billBeforeReceiveJson)}`
    );
    ...
    const firstReceiveRes = await adminRequest(`/api/orders/${orderId}/status`, {
      ...
      body: JSON.stringify({
        status: 'received',
        description: 'Smoke receive attempt with zero stock',
      }),
    });
    const firstReceiveJson = await toJson(firstReceiveRes);
    assert.equal(
      firstReceiveRes.status,
      200,
      `receive should allow partial fulfillment on low stock: ${JSON.stringify(firstReceiveJson)}`
    );
    assert.equal(
      Number(firstReceiveJson?.fulfilled_qty || 0),
      1,
      'fulfilled quantity should be limited by stock'
    );
    assert.equal(
      Number(firstReceiveJson?.pending_qty || 0),
      2,
      'pending quantity should reflect unfulfilled requested quantity'
    );
    ...
    assert.equal(
      productAfterReceiveRes.status,
      200,
      `product fetch after receive failed: ${JSON.stringify(productAfterReceiveJson)}`
    );
    assert.equal(
      Number(productAfterReceiveJson?.stock || 0),
      0,
      'stock should deduct only fulfilled quantity'
    );
    ...
    assert.equal(
      restockRes.status,
      200,
      `product restock before reapply failed: ${JSON.stringify(restockJson)}`
    );
    ...
    assert.equal(
      reapplyPendingRes.status,
      200,
      `reapply pending should succeed after restock: ${JSON.stringify(reapplyPendingJson)}`
    );
    assert.equal(
      Number(reapplyPendingJson?.fulfilled_qty || 0),
      3,
      'reapply pending should complete the remaining fulfillment'
    );
    assert.equal(
      Number(reapplyPendingJson?.pending_qty || 0),
      0,
      'reapply pending should clear pending quantity'
    );
    ...
    assert.equal(
      productAfterReapplyRes.status,
      200,
      `product fetch after reapply failed: ${JSON.stringify(productAfterReapplyJson)}`
    );
    assert.equal(
      Number(productAfterReapplyJson?.stock || 0),
      0,
      'reapply pending should consume the replenished stock'
    );
    ...
    assert.equal(
      ...
      400,
      `linked bill should reject an invalid order item reference: ${JSON.stringify(invalidLinkedBillJson)}`
    );
    ...
    assert.equal(
      ...
      201,
      `order-linked bill create failed: ${JSON.stringify(billCreateJson)}`
    );
    assert.equal(
      Boolean(billCreateJson?.stock_applied),
      false,
      'order-linked billing should not apply additional stock'
    );
    ...
    assert.equal(
      ...
      200,
      `bill detail fetch failed: ${JSON.stringify(persistedBillJson)}`
    );
    assert.equal(
      Number(persistedBillJson?.subtotal || 0),
      300,
      'bill subtotal should preserve pre-discount amount'
    );
    assert.equal(
      Number(persistedBillJson?.discount_amount || 0),
      30,
      'bill header discount should match applied offer total'
    );
    assert.equal(
      Number(persistedBillJson?.total_amount || 0),
      270,
      'bill total should reflect offer discount'
    );
    ...
    assert.equal(
      ...
      300,
      'bill item should persist line subtotal'
    );
    assert.equal(
      Number(persistedBillItem?.offer_discount || 0),
      30,
      'bill item should persist offer discount'
    );
    assert.equal(
      Number(persistedBillItem?.manual_discount || 0),
      0,
      'bill item should persist manual discount separately'
    );
    assert.equal(
      String(persistedBillItem?.offer_label || '').trim(),
      '10% OFF | First order only',
      'bill item should persist offer label'
    );
    assert.equal(
      Number(persistedBillItem?.discount || 0),
      30,
      'bill item discount should still expose combined discount'
    );
    assert.equal(
      Number(persistedBillItem?.amount || 0),
      270,
      'bill item amount should reflect discount'
    );
    ...
    assert.equal(
      productAfterBillRes.status,
      200,
      `product fetch after bill failed: ${JSON.stringify(productAfterBillJson)}`
    );
    assert.equal(
      Number(productAfterBillJson?.stock || 0),
      0,
      'stock should stay unchanged after order-linked billing'
    );
    ...
    assert.equal(
      ...
      200,
      `duplicate order bill should dedupe: ${JSON.stringify(duplicateBillJson)}`
    );
    assert.equal(
      Boolean(duplicateBillJson?.deduplicated),
      true,
      'duplicate order billing should be deduplicated'
    );
    ...
    assert.equal(
      customerCreditRes.status,
      200,
      `credit history fetch failed: ${JSON.stringify(customerCreditJson)}`
    );
    ...
    const creditRows = Array.isArray(customerCreditJson)
      ...
      : Array.isArray(customerCreditJson?.rows)
        ...
        : [];
    const creditMatch = creditRows.find(
      (entry) => String(entry?.reference || '').trim() === billReference
    );
    assert.equal(
      Boolean(creditMatch),
      true,
      'credit history should include order-linked bill reference'
    );
    ...
    assert.equal(
      ...
      200,
      `order detail fetch failed: ${JSON.stringify(orderDetailJson)}`
    );
    assert.equal(Boolean(orderDetailJson?.bill_id), true, 'order should expose linked bill_id');
    assert.equal(
      String(orderDetailJson?.payment_status || ''),
      'pending',
      'order payment_status should follow billing payment state'
    );
    assert.equal(
      Number(orderDetailJson?.items?.[0]?.stock || 0),
      0,
      'order detail should include current product stock for admin receive review'
    );
    ...
    assert.equal(
      secondOrderRes.status,
      201,
      `second order create failed: ${JSON.stringify(secondOrderJson)}`
    );
    assert.equal(
      Number(secondOrderJson?.totalAmount || 0),
      110,
      'second order should not receive first-order-only offer'
    );
    ...
    assert.equal(
      Number(secondOrderItem?.line_subtotal || 0),
      100,
      'second order subtotal should remain base price'
    );
    assert.equal(
      Number(secondOrderItem?.offer_discount || 0),
      0,
      'second order should not persist first-order discount'
    );
    assert.equal(
      String(secondOrderItem?.offer_label || '').trim(),
      '',
      'second order should not persist a first-order offer label'
    );
    assert.equal(
      Number(secondOrderItem?.total || 0),
      100,
      'second order line total should remain undiscounted'
    );
    ...
    assert.equal(
      previewAfterFirstOrderRes.status,
      200,
      `offer preview after first order failed: ${JSON.stringify(previewAfterFirstOrderJson)}`
    );
    assert.equal(
      Number(previewAfterFirstOrderJson?.summary?.auto_offer_discount_total || 0),
      0,
      'offer preview should stop applying first-order-only offer after first order'
    );
    assert.equal(
      String(previewAfterFirstOrderJson?.items?.[0]?.best_offer_label || '').trim(),
      '',
      'offer preview should clear first-order label after first order'
    );
    assert.equal(
      Number(previewAfterFirstOrderJson?.summary?.total || 0),
      110,
      'offer preview total should fall back to undiscounted price plus tax after first order'
    );
    ...
    assert.equal(
      adminBillingPreviewRes.status,
      200,
      `admin billing offer preview failed: ${JSON.stringify(adminBillingPreviewJson)}`
    );
    ...
    assert.equal(
      partialRestockRes.status,
      200,
      `product restock before partial available_now bill failed: ${JSON.stringify(partialRestockJson)}`
    );
    assert.equal(
      Number(partialRestockJson?.stock || 0),
      1,
      'restock should prepare one unit for partial available_now billing'
    );
    ...
    assert.equal(
      ...
      201,
      `partial available_now order create failed: ${JSON.stringify(partialOrderJson)}`
    );
    ...
    const partialReceiveRes = await adminRequest(`/api/orders/${partialOrderId}/status`, {
      ...
      body: JSON.stringify({
        status: 'received',
        description: 'Smoke receive before partial available_now billing',
      }),
    });
    ...
    assert.equal(
      ...
      200,
      `partial receive before available_now bill failed: ${JSON.stringify(partialReceiveJson)}`
    );
    assert.equal(
      Number(partialReceiveJson?.fulfilled_qty || 0),
      1,
      'partial available_now order should only fulfill one unit'
    );
    assert.equal(
      Number(partialReceiveJson?.pending_qty || 0),
      1,
      'partial available_now order should keep one pending unit'
    );
    ...
    assert.equal(
      ...
      201,
      `partial available_now bill create failed: ${JSON.stringify(partialBillJson)}`
    );
    assert.equal(
      Number(partialBillJson?.fulfilled_qty || 0),
      1,
      'partial available_now bill response should report one billed unit'
    );
    assert.equal(
      Number(partialBillJson?.pending_qty || 0),
      1,
      'partial available_now bill response should report one pending unit'
    );
    ...
    assert.equal(
      ...
      200,
      `partial available_now bill fetch failed: ${JSON.stringify(partialPersistedBillJson)}`
    );
    assert.equal(
      Number(partialPersistedBillJson?.subtotal || 0),
      100,
      'partial available_now bill subtotal should only charge fulfilled quantity'
    );
    assert.equal(
      Number(partialPersistedBillJson?.discount_amount || 0),
      0,
      'partial available_now bill should not invent a discount'
    );
    assert.equal(
      Number(partialPersistedBillJson?.total_amount || 0),
      100,
      'partial available_now bill total should only charge fulfilled quantity'
    );
    const partialBillItem = Array.isArray(partialPersistedBillJson?.items)
      ? partialPersistedBillJson.items[0]
      : null;
    assert.equal(
      Number(partialBillItem?.qty || 0),
      1,
      'partial available_now bill item qty should match fulfilled quantity'
    );
    assert.equal(
      Number(partialBillItem?.requested_qty || 0),
      2,
      'partial available_now bill item should keep requested quantity for traceability'
    );
    assert.equal(
      Number(partialBillItem?.fulfilled_qty || 0),
      1,
      'partial available_now bill item should persist fulfilled quantity'
    );
    assert.equal(
      Number(partialBillItem?.pending_qty || 0),
      1,
      'partial available_now bill item should persist pending quantity'
    );
    assert.equal(
      Number(partialBillItem?.line_subtotal || 0),
      100,
      'partial available_now bill item subtotal should only charge the fulfilled quantity'
    );
    assert.equal(
      Number(partialBillItem?.amount || 0),
      100,
      'partial available_now bill item amount should only charge the fulfilled quantity'
    );
