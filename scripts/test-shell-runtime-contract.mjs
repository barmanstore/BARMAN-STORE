...
const expectNoViolations = (source, relativePath, label) => {
  const violations = collectPathnamePredicateViolations({
    source: wrapSnippet(source),
    relativePath,
  });
  assert.equal(
    violations.length,
...
const runPathnameVerification = () => {
  ...
  expectNoViolations(
    'const url = `${canonicalHost}${location.pathname}`;',
    blocklistedFile,
    'Passive string construction'
  );
  expectNoViolations(
  ...
  expectNoViolations(
    ...
    uiFile,
    'UI pathname read outside blocklist'
  );
  ...
  expectViolations(
    "switch (pathname) { case '/admin': break; default: break; }",
    blocklistedFile,
    'Switch predicate'
  );
  expectViolations(
    "pathname.startsWith('/popup') && setVariant('none');",
    blocklistedFile,
    '&& side effect'
  );
  expectViolations(
    "!pathname.includes('/store') || activateChrome();",
    blocklistedFile,
    '|| side effect'
  );
  expectViolations(
  ...
  expectViolations(
    "const config = { isPopup: pathname.startsWith('/popup') };",
    blocklistedFile,
    'Property predicate'
  );
  expectViolations("return pathname.startsWith('/popup');", blocklistedFile, 'Return predicate');
  expectViolations(
    'if (/^\\/popup/.test(pathname)) { closeShell(); }',
    blocklistedFile,
    'Regex test predicate'
  );
};
...
const runOverlayVerification = () => {
  ...
  assert.equal(
    selectTopEscapeEntry([sheetEntry, popupEntry])?.id,
    'popup',
    'Higher overlay z-index should win.'
  );
  assert.equal(
    shouldLockOverlayBackground([mobileMenuEntry]),
    false,
    'Chrome-only stacks must not lock the background.'
  );
  assert.equal(
    shouldLockOverlayBackground([mobileMenuEntry, sheetEntry]),
    true,
    'Overlay stacks must lock the background.'
  );
  ...
  assert.equal(
    selectTopEscapeEntry([sheetEntry, dynamicWindowEntry])?.id,
    'window',
    'Dynamic window z-index should outrank the sheet.'
  );
  order = 12;
  assert.equal(
    selectTopEscapeEntry([sheetEntry, dynamicWindowEntry])?.id,
    'window',
    'Dynamic getZIndex should be evaluated at routing time.'
  );

  const olderChromeEntry = {
    id: 'older',
    type: 'chrome',
    zIndex: 1600,
    onEscape() {},
  ...
  assert.equal(
    selectTopEscapeEntry([olderChromeEntry, newerChromeEntry])?.id,
    'newer',
    'Sequence should break equal-z-index ties.'
  );
};
...
const runInertVerification = () => {
  ...
  assert.equal(
    ...
    'true',
    'Background should be aria-hidden on first lock.'
  );
  ...
  assert.equal(
    background.inert,
    true,
    'Background should remain inert while one overlay is still open.'
  );
  releaseWindow();
  ...
  assert.equal(
    background.getAttribute('aria-hidden'),
    null,
    'Last unlock should restore aria-hidden.'
  );
  ...
  releaseFirst();
  assert.equal(
    overlapBackground.inert,
    true,
    'Intermediate unlock must not re-enable the background.'
  );
  releaseSecond();
  ...
  assert.equal(
    priorStateBackground.style.pointerEvents,
    'none',
    'Prior pointer-events state must restore exactly.'
  );
  assert.equal(
    priorStateBackground.getAttribute('aria-hidden'),
    'true',
    'Prior aria-hidden state must restore exactly.'
  );
  ...
  assert.equal(
    fallbackBackground.style.pointerEvents,
    'none',
    'Fallback mode should use pointer-events locking.'
  );
  releaseFallback();
  assert.equal(
    fallbackBackground.style.pointerEvents,
    '',
    'Fallback mode should restore pointer-events.'
  );
  ...
  releaseA();
  assert.equal(
    freshSnapshotState.snapshot,
    null,
    'Snapshot must clear after a no-overlap session.'
  );
  freshSnapshotBackground.inert = true;
  const releaseB = acquireInertBackgroundLock(freshSnapshotState, freshSnapshotBackground);
  releaseB();
  assert.equal(
    freshSnapshotBackground.inert,
    true,
    'Fresh sessions must restore against a fresh snapshot.'
  );
};
