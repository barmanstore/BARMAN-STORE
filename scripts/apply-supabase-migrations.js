...
const main = async () => {
  ...
  if (mode !== 'postgres') {
    console.log(`[DB] DB_EXECUTION_MODE=${mode}. Set DB_EXECUTION_MODE=postgres to apply Supabase/Postgres migrations.`);
    return;
  }
  ...
  try {
    ...
    if (result.applied.length) {
      console.log(`[DB] Applied migrations (${result.applied.length}/${result.total}): ${result.applied.join(', ')}`);
    } else {
