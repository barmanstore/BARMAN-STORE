...
const { resolveCreditTermsDays } = require('../server/features/credits/utils/creditStatusPolicy.js');
...
const runTests = () => {
  ...
  console.log(`Test 3 (transaction_ts): transaction_ts=${timedTx.transaction_ts}, created_at=${timedTx.created_at}`);
  console.log(`Result: dateKey=${timedDateKey}, timestamp=${new Date(timedTimestamp).toISOString()}`);
  assert.equal(timedDateKey, '2026-03-07', 'Should use transaction_ts date before created_at');
  assert.equal(new Date(timedTimestamp).toISOString(), '2026-03-07T15:45:00.000Z', 'Should use transaction_ts timestamp before created_at');
  ...
  console.log(`Test 5 (status windows): new=${newStatusDays}, good=${goodStatusDays}, override=${manualOverrideDays}`);
